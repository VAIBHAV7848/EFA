...
1. **`claude agents` command** (primary) — run `claude agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `everything-for-ai:architect`), and built-in agents. This automatically covers EFA marketplace installs without any path configuration.
...
- **Plugin agents** are prefixed with `plugin-name:` (e.g., `everything-for-ai:security-reviewer`). Use the part after `:` as the agent name and the plugin name as the domain.
...
- **Parallel Agent calls, not TeamCreate.** This skill uses parallel Agent tool calls for independent work. TeamCreate (a Everything For Ai tool for multi-agent dialogue) is only needed when agents must debate or respond to each other.
