...
1. Install EFA as your plugin (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use AgentShield for security audits
4. Use EFA's verification-loop as a final gate before PRs

### Avoiding Hook Conflicts

If running both EFA and Plankton hooks:
- EFA's Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable EFA's Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (EFA handles what Plankton doesn't cover)
...
```bash
export EFA_HOOK_PROFILE=strict
export EFA_QUALITY_GATE_FIX=true
export EFA_QUALITY_GATE_STRICT=true
```
