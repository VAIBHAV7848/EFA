...
**Opt-in (EFA):** The `codescene` block in `mcp-configs/mcp-servers.json` is a template only. EFA plugin installs do not auto-enable bundled MCP servers. Copy the entry into your config only if you want it. You can exclude it during EFA install/sync with `EFA_DISABLED_MCPS=codescene,...`.
...
Same triggers as **When to Use** above — this heading is what EFA uses for skill auto-activation.
...
| EFA skill / flow | Code Health MCP role |
|------------------|----------------------|
...
**Context tip:** EFA recommends keeping MCP count low. Enable `codescene` when doing substantive edits; disable when not needed.
