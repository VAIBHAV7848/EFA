---
description: Run the EFA formatter quality gate for a single file and report remediation steps.
---
...
- Behavior toggles are environment variables:
  - `EFA_QUALITY_GATE_FIX=true` - apply formatting fixes instead of check-only
  - `EFA_QUALITY_GATE_STRICT=true` - log formatter failures as gate failures
...
echo '{"tool_input":{"file_path":"src/example.ts"}}' \
  | EFA_QUALITY_GATE_FIX=true node scripts/hooks/quality-gate.js
```
