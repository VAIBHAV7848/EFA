...
This document captures architect-level improvements for the Everything For Ai (EFA) project. It is written from the perspective of a Claude Code coding architect aiming to improve maintainability, consistency, and long-term quality.
...
- Ensure `EFA_HOOK_PROFILE` and `EFA_DISABLED_HOOKS` are respected in all code paths (including shell) so behavior is consistent.
