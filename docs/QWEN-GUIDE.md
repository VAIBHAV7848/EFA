...
EFA can install its managed command, agent, skill, rule, and MCP surfaces into the Qwen CLI home directory.

## Install

From the EFA repository root:
...
The Qwen adapter writes into `~/.qwen/` and records managed file ownership in `~/.qwen/efa-install-state.json`.
...
Rerun the same install command after pulling EFA updates. The installer uses the install-state file to update EFA-managed files without claiming unrelated user files in `~/.qwen/`.
...
That removes files recorded in `~/.qwen/efa-install-state.json` and leaves unrelated Qwen configuration alone.
