---
description: プロジェクトのスタックを検出し、リポジトリのインストールマニフェストとスタックマッピングを使用してドライランEFAオンボーディング計画を生成します。
---

# /project-init

現在のプロジェクト用の安全でレビュー可能なEFAオンボーディング計画を作成します。このコマンドはドライランモードで開始し、明示的なユーザー承認後にのみファイルを書き込みます。
...
3. EFAのインストーラーとマニフェストツールを使用。インストールのショートカットとしてファイルを手動コピーしたり任意のリモートをクローンしない。
...
- EFA設定: `efa-install.json`
- オプションのスタックマップ: EFAリポジトリ内の`config/project-stack-mappings.json`

EFAチェックアウトが利用可能な場合、`config/project-stack-mappings.json`をスタックからルール/スキルへの参照として使用。ファイルが利用できない場合、インストール済みのEFAマニフェストと明示的なユーザーの選択にフォールバック。
...
3. 最小限の有用なEFA計画を解決:
   - プロジェクトに`efa-install.json`がある: `node scripts/install-plan.js --config efa-install.json --json`
...
- `/efa-guide` — インストール前のインタラクティブな機能ディスカバリー
