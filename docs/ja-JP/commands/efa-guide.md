---
description: EFAの現在のエージェント、スキル、コマンド、フック、インストールプロファイル、ドキュメントをライブリポジトリサーフェスからナビゲートします。
---

# /efa-guide

このコマンドをEverything For Aiの会話型マップとして使用します。完全なREADMEや古いカタログ数をダンプすることなく、タスクに適したEFAサーフェスをユーザーが発見するのを支援します。
...
- `scripts/ci/catalog.js --json` — EFA内で実行時のライブカタログカウント
...
- `/project-init` — スタック対応のEFAオンボーディング
