...
- **instinctインポート時のコンテンツ喪失を修正** — `/instinct-import`実行時に`parse_instinct_file()`がfrontmatter後のすべてのコンテンツ（Action、Evidence、Examplesセクション）を暗黙的に削除していた問題を修正。コミュニティ貢献者@ericcai0814により解決されました（[#148](https://github.com/VAIBHAV7848/EFA/issues/148), [#161](https://github.com/VAIBHAV7848/EFA/pull/161)）

### v1.4.0 — マルチ言語ルール、インストールウィザード & PM2（2026年2月）

- **インタラクティブインストールウィザード** — 新しい`configure-efa`スキルがマージ/上書き検出付きガイドセットアップを提供
...
完全なチェンジログは[Releases](https://github.com/VAIBHAV7848/EFA/releases)を参照してください。
...
cp -r everything-for-ai/rules/typescript ~/.claude/rules/typescript
...
|   |-- configure-efa/              # インタラクティブインストールウィザード（新規）
...
[GitHub アプリをインストール](https://github.com/apps/skill-creator) | [efa.tools](https://efa.tools)
...
Claude Codeで`/security-scan`を実行、または[GitHub Action](https://github.com/VAIBHAV7848/agentshield)でCIに追加できます。
...
**背景:** これは本リポジトリで複数の修正/リバート循環を引き起こしました（[#29](https://github.com/VAIBHAV7848/EFA/issues/29), [#52](https://github.com/VAIBHAV7848/EFA/issues/52), [#103](https://github.com/VAIBHAV7848/EFA/issues/103)）。Claude Codeバージョン間で動作が変わったため混乱がありました。今後を防ぐため回帰テストがあります。
...
> cp -r everything-for-ai/rules/common ~/.claude/rules/common
> cp -r everything-for-ai/rules/typescript ~/.claude/rules/typescript   # スタックを選択
> cp -r everything-for-ai/rules/python ~/.claude/rules/python
> cp -r everything-for-ai/rules/golang ~/.claude/rules/golang
>
...
# スキルをコピー
cp -r everything-for-ai/skills/* ~/.claude/skills/
```
...
`/plugin install` で EFA を導入した場合は、これらのフックを `settings.json` にコピーしないでください。Claude Code v2.1+ はプラグインの `hooks/hooks.json` を自動読み込みするため、二重登録すると重複実行や `${CLAUDE_PLUGIN_ROOT}` の解決失敗が発生します。
...
efa-universal は [Cursor IDE](https://cursor.com) の事前翻訳設定を含みます。`.cursor/` ディレクトリには、Cursor フォーマット向けに適応されたルール、エージェント、スキル、コマンド、MCP 設定が含まれています。
...
EFAは**フルOpenCodeサポート**をプラグインとフック含めて提供。
...
- **簡潔ガイド（まずはこれ）:** [Everything For Ai 簡潔ガイド](https://x.com/VAIBHAV7848/status/2012378465664745795)
- **詳細ガイド（高度）:** [Everything For Ai 詳細ガイド](https://x.com/VAIBHAV7848/status/2014040193557471352)
- **フォロー:** [@VAIBHAV7848](https://x.com/VAIBHAV7848)
