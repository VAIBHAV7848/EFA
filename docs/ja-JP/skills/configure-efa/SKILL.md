---
name: configure-efa
description: Everything For Ai のインタラクティブなインストーラー — スキルとルールの選択とインストールをユーザーレベルまたはプロジェクトレベルのディレクトリへガイドし、パスを検証し、必要に応じてインストールされたファイルを最適化します。
---

# Configure Everything For Ai (EFA)

Everything For Ai プロジェクトのインタラクティブなステップバイステップのインストールウィザードです。`AskUserQuestion` を使用してスキルとルールの選択的インストールをユーザーにガイドし、正確性を検証し、最適化を提供します。

## 起動タイミング

- ユーザーが "configure efa"、"install efa"、"setup everything for ai" などと言った場合
- ユーザーがこのプロジェクトからスキルまたはルールを選択的にインストールしたい場合
- ユーザーが既存の EFA インストールを検証または修正したい場合
- ユーザーがインストールされたスキルまたはルールをプロジェクト用に最適化したい場合

## 前提条件

このスキルは起動前に Claude Code からアクセス可能である必要があります。ブートストラップには2つの方法があります：
1. **プラグイン経由**: `/plugin install efa@efa` — プラグインがこのスキルを自動的にロードします
2. **手動**: このスキルのみを `~/.claude/skills/configure-efa/SKILL.md` にコピーし、"configure efa" と言って起動します

---

## ステップ 0: EFA リポジトリのクローン

インストールの前に、最新の EFA ソースを `/tmp` にクローンします：
...
以降のすべてのコピー操作のソースとして `EFA_ROOT=/tmp/everything-for-ai` を設定します。

クローンが失敗した場合（ネットワークの問題など）、`AskUserQuestion` を使用してユーザーに既存の EFA クローンへのローカルパスを提供するよう依頼します。
...
```
Question: "EFA コンポーネントをどこにインストールしますか？"
Options:
...
```bash
# 共通ルール
cp -r $EFA_ROOT/rules/common $TARGET/rules/common

# 言語固有のルール（言語別ディレクトリを保持）
cp -r $EFA_ROOT/rules/typescript $TARGET/rules/typescript   # 選択された場合
cp -r $EFA_ROOT/rules/python $TARGET/rules/python            # 選択された場合
cp -r $EFA_ROOT/rules/golang $TARGET/rules/golang            # 選択された場合
```
...
**重要**: インストール先（`$TARGET/`）のファイルのみを変更し、ソース EFA リポジトリ（`$EFA_ROOT/`）のファイルは決して変更しないでください。
