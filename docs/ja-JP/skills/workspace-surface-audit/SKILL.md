...
description: アクティブなリポジトリ、MCPサーバー、プラグイン、コネクター、環境サーフェス、ツールのセットアップを監査し、最も価値の高いEFAネイティブスキル、フック、エージェント、オペレーターワークフローを推奨する。ユーザーがClaude Codeのセットアップを支援してほしい場合や、環境で実際に何が使えるかを理解したい場合に使用する。
origin: EFA
---
...
* EFAがそのサーフェスを合理的に所有できる場合は、一般的な「別のプラグインをインストール」という推奨よりEFAネイティブワークフローを優先する。
* 外部プラグインをベースラインとインスピレーションとして扱い、権威ある製品境界としてではない。
* 3つのことを明確に区別する：
  * 現在利用可能なもの
  * 利用可能だがEFAのカプセル化が不十分なもの
  * 利用不可能で新しい統合が必要なもの
...
2. 環境サーフェス
   * アクティブなリポジトリと明らかに隣接するEFAワークスペース内の `.env*` ファイル
...
* 設定済みのLSPサーバー
* キー名で示唆される環境ベースのサービス
* ワークスペースに関連する既存のEFAスキル

サーフェスが生の形式でしか存在しない場合は指摘する。例：

* 「Stripeは接続されたアプリで利用可能だが、EFAには課金操作スキルがない」
* 「Google Driveは接続されているが、EFAにはGoogle Workspaceネイティブの操作ワークフローがない」
...
1. それらが実際に何をするか
2. EFAが同等の機能をすでに持っているか
3. EFAが生の形式のみを持っているか
4. EFAがそのワークフローを完全に欠いているか

### フェーズ3：ギャップをEFAの決定に変換する

各実際のギャップについて、適切なEFAネイティブの形式を推奨する：
...
1. **現在のサーフェス**
   * 現在利用可能なもの
2. **同等の機能**
   * EFAがベースラインに一致または超えている場所
3. **生の形式のみのギャップ**
   * ツールは存在するが、EFAには簡潔な操作スキルがない
4. **欠けている統合**
   * まだ利用できない機能
5. **上位3-5の次のステップ**
   * 影響度順の具体的なEFAネイティブな追加
...
* EFAがすでに強力な生の形式を持っている場合は、まったく新しいサブシステムを発明するのではなくカプセル化スキルを提案する。

## 良い結果

* ユーザーが接続されているもの、欠けているもの、EFAが次に持つべきものをすぐに確認できる。
