...
[AgentShield](https://github.com/VAIBHAV7848/agentshield) を使用して、Claude Code の設定のセキュリティ問題を監査します。
...
```bash
# インストール済みか確認
npx efa-agentshield --version

# グローバルにインストール（推奨）
npm install -g efa-agentshield
...
# 特定のパスをスキャン
npx efa-agentshield scan --path /path/to/.claude

# 最小深刻度フィルタでスキャン
npx efa-agentshield scan --min-severity medium
```
...
# JSON — CI/CD 統合用
npx efa-agentshield scan --format json

# Markdown — ドキュメント用
npx efa-agentshield scan --format markdown

# HTML — 自己完結型のダークテーマレポート
npx efa-agentshield scan --format html > security-report.html
```
