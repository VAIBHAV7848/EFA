...
- HermesオペレーターストーリーのためのパブリックEFA 2.0リリース候補サーフェスを追加。
- Claude Code、Codex、Cursor、OpenCode、Gemini全体で再利用可能なクロスハーネス基盤としてEFAをドキュメント化。
...
- `docs/architecture/cross-harness.md` とEFA/Hermesバウンダリのリグレッションカバレッジを追加。
- `efa2/` のバージョニングは現時点では独立を維持；リリースエンジニアリングが別途決定しない限り、アルファコントロールプレーンのスキャフォールドのまま。

### 注記

- これはリリース候補であり、完全なEFA 2.0コントロールプレーンロードマップのGA宣言ではありません。
...
- EFA 2.0アルファコントロールプレーンバイナリが `efa2/` からローカルビルド可能になり、最初の使用可能なCLI/TUIサーフェスを公開。
...
- `cargo build --manifest-path efa2/Cargo.toml` がリポジトリのベースラインで通過。
- `efa-tui` は現在 `dashboard`、`start`、`sessions`、`status`、`stop`、`resume`、`daemon` を公開。
...
- このリリースはリポジトリサーフェスの修正とエコシステム同期であり、完全なEFA 2.0ロードマップが完成したという主張ではありません。
...
- `efa-devfleet` — マルチエージェントオーケストレーション (#505)
- `blueprint` — マルチセッション構築計画
- `everything-for-ai` — 自己参照型EFAスキル (#335)
...
- `EFA_HOOK_PROFILE` と `EFA_DISABLED_HOOKS` 制御を追加。
