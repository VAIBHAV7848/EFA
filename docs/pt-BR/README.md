...
### v2.0.0-rc.1 — Sincronização de Superfície, Fluxos Operacionais e EFA 2.0 Alpha (Abr 2026)
...
- **EFA 2.0 alpha já está no repositório** — o plano de controle em Rust dentro de `efa2/` já compila localmente e expõe `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` e `daemon`.
- **Fortalecimento do ecossistema** — AgentShield, controles de custo do EFA Tools, trabalho no portal de billing e a renovação do site continuam sendo entregues ao redor do plugin principal.
...
- **Lançamento focado em harness** — O EFA agora é explicitamente enquadrado como um sistema de desempenho de harness de agentes, não apenas um pacote de configurações.
...
- **Controles de runtime de hooks** — `EFA_HOOK_PROFILE=minimal|standard|strict` e `EFA_DISABLED_HOOKS=...` para controle em tempo de execução sem editar arquivos de hook.
...
/efa:plan "Adicionar autenticação de usuário"
...
> cp -r everything-for-ai/rules/typescript ~/.claude/rules/typescript   # escolha sua stack
>
...
|----------|-----------------|--------------|
| Planejar um novo recurso | `/efa:plan "Adicionar auth"` | planner |
| Projetar arquitetura de sistema | `/efa:plan` + agente architect | architect |
...
```
/efa:plan "Adicionar autenticação de usuário com OAuth"
...
<details>
<summary><b>Posso usar o EFA com Cursor / OpenCode / Codex / Antigravity?</b></summary>

Sim. O EFA é multiplataforma:
