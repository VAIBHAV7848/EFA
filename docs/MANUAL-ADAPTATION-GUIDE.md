...
Use this guide when you want EFA behavior inside a harness that does not natively load `.claude/`, `.codex/`, `.opencode/`, `.cursor/`, or `.agent/` layouts.
...
Prefer a first-class EFA target whenever one exists:
...
Use this guide only when you need EFA behavior in a non-native harness.

## What You Are Reproducing

When you adapt EFA manually, you are trying to preserve four things:
...
```bash
cd /path/to/everything-for-ai

sed -n '1,220p' skills/tdd-workflow/SKILL.md > /tmp/efa-context.md
printf '\n\n---\n\n' >> /tmp/efa-context.md
sed -n '1,220p' skills/backend-patterns/SKILL.md >> /tmp/efa-context.md
printf '\n\n---\n\n' >> /tmp/efa-context.md
sed -n '1,220p' skills/security-review/SKILL.md >> /tmp/efa-context.md
```
...
Optional: if you already use a repo packer like `repomix`, it can help compress selected files into one handoff document. It is a convenience tool, not the canonical EFA path.

## Compression Rules

When manually packing EFA for another harness:
...
You are not implementing real commands. You are giving the harness explicit invocation handles that map to EFA behavior.
...
That does not recreate true automation, but it captures the operational discipline of EFA.

## Harness Capability Matrix

| Capability | First-Class EFA Targets | Manual-Adaptation Targets |
| --- | --- | --- |
...
2. Pack the selected EFA skill files into one upload or paste block.
...
```text
You are operating with a manually adapted EFA bundle.
...
- use manual adaptation to carry EFA behavior into non-native harnesses
- use native EFA targets whenever you want the full system

## Related Work

- [Issue #1186](https://github.com/VAIBHAV7848/EFA/issues/1186)
- [Discussion #1077](https://github.com/VAIBHAV7848/EFA/discussions/1077)
