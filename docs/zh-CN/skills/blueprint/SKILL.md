...
此技能随 Everything For Ai 附带。安装 EFA 时无需单独安装。

### 完整 EFA 安装

如果您从 EFA 仓库检出中工作，请验证技能是否存在：

```bash
test -f skills/blueprint/SKILL.md
```

后续更新时，请在更新前查看 EFA 的差异：
...
如果您在完整 EFA 安装之外仅内嵌此技能，请将 EFA 仓库中已审查的文件复制到 `~/.claude/skills/blueprint/SKILL.md`。内嵌副本没有 git 远程仓库，因此应通过从已审查的 EFA 提交中重新复制文件来更新，而不是运行 `git pull`。
