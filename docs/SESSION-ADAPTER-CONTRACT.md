...
This document defines the canonical EFA session snapshot contract for
`efa.session.v1`.
...
EFA has multiple session sources:
...
| Field | Type | Notes |
| --- | --- | --- |
| `schemaVersion` | string | MUST be exactly `efa.session.v1` for this contract |
...
Every EFA session adapter MUST:

1. Emit `schemaVersion: "efa.session.v1"` exactly.
...
- rely only on documented fields for `efa.session.v1`
