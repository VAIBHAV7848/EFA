...
- what EFA owns and may safely remove or repair later

That is the missing contract between EFA 1.x installation and an EFA 2.0
control plane.
...
- unified `efa` CLI routing install, planning, and lifecycle commands
...
- legacy `efa-install` compatibility still points at `install.sh`
...
- the repo already has a unified Node entrypoint through `efa` and
  `install-apply.js`
...
EFA 2.0 should make operation planning a standalone phase so that:
...
- `uninstall`: remove only EFA-owned outputs
...
- `scripts/efa.js` as the unified CLI
...
If the goal is EFA 2.0 and not just “working enough,” the next modularization
steps should be:
...
Today EFA still behaves like a broad payload copier:
...
That means EFA 2.0 needs two contracts, not one:
...
1. Keep `everything-for-ai` as the canonical source repo.
...
Fields still needed for EFA 2.0:

- `installStrategy`
  for example `copy`, `flatten-rules`, `generate`, `merge-config`
- `ownership`
  whether EFA fully owns the target path or only generated files under it
- `pathMode`
...
That lets EFA 2.0 say things like:
...
EFA 2.0 needs the next layer: operation planning.
...
```json
{
  ...
  "destination": "/Users/example/.claude/rules/efa/common/coding-style.md",
  "ownership": "managed",
...
Install-state is the durable contract that EFA 1.x is missing.

Suggested path conventions:

- Claude target:
  `~/.claude/efa/install-state.json`
- Cursor target:
  `./.cursor/efa-install-state.json`
- Antigravity target:
  `./.agent/efa-install-state.json`
- future Codex target:
  `~/.codex/efa-install-state.json`
...
- enough detail for uninstall to remove only EFA-managed outputs
...
- `efa list-installed` routes to `node scripts/list-installed.js`
- `efa uninstall` routes to `node scripts/uninstall.js`
- `efa doctor` routes to `node scripts/doctor.js`
- `efa repair` routes to `node scripts/repair.js`
...
- load install-state
- remove only EFA-managed destinations recorded in state
...
EFA 2.0 should translate legacy language arguments into a compatibility request.
...
EFA 2.0 should improve this carefully.
...
3. define `efa.install.v1` state schema
...
4. decide when `efa-install` should become a thin alias for `efa install`
...
3. reduce package shipping surface to the module graph instead of broad folders
4. decide whether `efa-install` remains separate or becomes `efa install`
5. add tests that lock down:
   - target-specific merge/remove behavior
   - repair and uninstall safety for non-copy operations
   - unified `efa` CLI routing and compatibility guarantees
...
That is the shortest path from EFA 1.x installer sprawl to an EFA 2.0
