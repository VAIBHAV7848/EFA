...
Google's [Antigravity](https://antigravity.dev) is an AI coding IDE that uses a `.agent/` directory convention for configuration. EFA provides first-class support for Antigravity through its selective install system.

## Quick Start

```bash
# Install EFA with Antigravity target
...
This installs EFA components into your project's `.agent/` directory, ready for Antigravity to pick up.

## How the Install Mapping Works

EFA remaps its component structure to match Antigravity's expected layout:

| EFA Source | Antigravity Destination | What It Contains |
|------------|------------------------|------------------|
...
- **Commands become workflows**: EFA's `/command` files land in `.agent/workflows/`, which is Antigravity's equivalent of slash commands.
- **Agents become skills**: EFA agent definitions map to `.agent/skills/`, where Antigravity looks for skill configurations.
...
│   │   ├── tdd-guide.md
│   │   └── ...
│   └── efa-install-state.json     # tracks what EFA installed
```
...
The installer writes `.agent/efa-install-state.json` to track which files EFA owns. This enables safe uninstall and repair — EFA will never touch files it didn't create.
...
| Install state | `efa-install-state.json` | `efa-install-state.json` | `efa-install-state.json` | `efa-install-state.json` |
...
- Check that `efa-install-state.json` was created — if missing, re-run the installer
...
- If you manually copied EFA commands, rename the directory
