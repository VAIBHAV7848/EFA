...
`everything-for-ai` pull-request queue as of `2026-03-13T08:33:31Z`.
...
- targeted hook invocations for:
  ...
  - `EFA_SKIP_OBSERVE=1`
  - `agent_id` payload
  - trimmed `EFA_OBSERVE_SKIP_PATHS`
...
2. Low: the new guard matrix shipped without direct regression coverage.
   ...
   directly assert the new `CLAUDE_CODE_ENTRYPOINT`, `EFA_HOOK_PROFILE`,
   `EFA_SKIP_OBSERVE`, `agent_id`, or trimmed skip-path branches.
