# Hermes / OpenClaw -> EFA Migration

This document is the public migration guide for moving a Hermes or OpenClaw-style operator setup into the current EFA model.
...
EFA is the durable public system:
...
- install surfaces
- session adapters
- EFA 2.0 control-plane work

Hermes and OpenClaw are useful inputs because they contain repeated operator workflows that can be distilled into EFA-native surfaces.

That means the shortest safe path is:

1. extract the reusable behavior
2. translate it into EFA-native skills, hooks, docs, or adapter work
...
- EFA hook / command automation for local repeatability
- EFA 2.0 scheduler work under issue `#1050`
...
- EFA 2.0 as the future long-lived scheduling/control plane
...
- EFA session adapter and control-plane work
- orchestration/session inspection commands
- EFA 2.0 control-plane backlog under:
  - `#1045`
  - `#1046`
  - `#1047`
  - `#1048`
...
- EFA-native top-level skills when the workflow is reusable
...
- EFA-native operator skills when the workflow logic is the real asset
- adapter/control-plane work when the missing piece is session/runtime coordination

Do not import opaque third-party runtimes into EFA just because a private workflow depended on them.

If a workflow is valuable:

1. understand the behavior
2. rebuild the minimum EFA-native version
...
- EFA 2.0 adapter/control-plane discovery docs
...
- continuing the EFA 2.0 operator/control-plane buildout

EFA 2.0 now ships a bounded migration audit entrypoint:

- `efa migrate audit --source ~/.hermes`
- `efa migrate plan --source ~/.hermes --output migration-plan.md`
- `efa migrate scaffold --source ~/.hermes --output-dir migration-artifacts`
- `efa migrate import-skills --source ~/.hermes --output-dir migration-artifacts/skills`
- `efa migrate import-tools --source ~/.hermes --output-dir migration-artifacts/tools`
- `efa migrate import-plugins --source ~/.hermes --output-dir migration-artifacts/plugins`
- `efa migrate import-schedules --source ~/.hermes --dry-run`
- `efa migrate import-remote --source ~/.hermes --dry-run`
- `efa migrate import-env --source ~/.hermes --dry-run`
- `efa migrate import-memory --source ~/.hermes`

Use that first to inventory the legacy workspace and map detected surfaces onto the current EFA2 scheduler, remote dispatch, memory graph, templates, and manual-translation lanes.
...
1. Keep the public EFA repo as the canonical reusable layer.
2. Port reusable Hermes/OpenClaw workflows into EFA-native skills one lane at a time.
3. Keep private auth and personal context outside the repo.
4. Use GitHub / Linear / KB systems as durable truth.
5. Treat EFA 2.0 as the path to a native operator shell, not as a finished product.
