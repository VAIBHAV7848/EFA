# EFA 2.0 Observability Readiness

EFA 2.0 should be observable before it becomes more autonomous. The local
...
- Tool activity: `scripts/hooks/session-activity-tracker.js` records local
  `tool-usage.jsonl` events that EFA2 can sync.
- Risk ledger: `efa2/src/observability/mod.rs` scores tool calls and stores a
...
EFA's answer is not a hosted analytics dependency by default. The first
...
2. Run `npm run harness:audit -- --format json` for the broader harness
   scorecard.
3. Run `node scripts/loop-status.js --json --write-dir .efa/loop-status`
...
7. Use EFA2 tool logs for risky operations, conflict analysis, and handoff
...
The end-state is practical: before asking EFA to run larger multi-agent loops,
