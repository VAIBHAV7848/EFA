...
EFA is the reusable workflow layer. Harnesses are execution surfaces.
...
[EFA Platform Value Loop](platform-value-loop.md).
...
| Sessions | `efa2/`, session adapters, orchestration scripts | TUI/daemon, tmux/worktree orchestration, harness-specific runners | Alpha |

## What Travels Unchanged

`SKILL.md` is the most portable unit.

A good EFA skill should:
...
- OpenCode has a plugin/event system that can reuse EFA hook logic through an adapter layer.
- Cursor uses its own rule and hook layout, so EFA maintains translated surfaces under `.cursor/`.
...
Hermes is not the public EFA runtime.

Hermes is an operator shell that can consume EFA assets:

- import selected EFA skills into a Hermes skills directory
- use EFA MCP conventions for tool access
- route chat, CLI, cron, and handoff workflows through reusable EFA patterns
- distill repeated local operator work back into sanitized EFA skills
...
OpenCode gets the skill through the OpenCode package/plugin surface. Event handling can reuse EFA hook logic through the adapter layer, while the skill text stays unchanged.
...
- the full platform loop where external products contribute skill packs,
  gated APIs, evals, and case studies back into EFA

## Rule For New Work

When adding a workflow, put the durable behavior in EFA first.
