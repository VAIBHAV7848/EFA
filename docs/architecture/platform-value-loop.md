# EFA Platform Value Loop

EFA 2.0 is moving from a portable harness layer toward a full operator
...
2. Dedicated EFA agent: an agent that directly operates over EFA assets instead
...
```text
EFA can be used full-stack as a meta-harness + agent + control pane, or
...
- product-specific integrations that can become reusable EFA skills.
...
External products can build on EFA without becoming EFA-branded products. The
contract is:

| Layer | Product contributes | EFA receives |
| --- | --- | --- |
...
| Case study | A real product workflow that works through EFA | Distribution, sponsors, Pro interest, consulting demand |
...
Ito is a separate prediction-market basket product. EFA can still distribute
Ito-shaped skills because the skill workflows are useful without making EFA
...
The boundary is strict: public EFA skills do not place trades, do not provide investment advice, do not expose private strategy, and do not merge EFA Tools billing with Ito billing.

## Value Loop

The platform loop should be explicit:

1. A product team builds a useful workflow as an EFA skill pack.
...
5. Sanitized patterns become better EFA skills, evals, gates, or docs.
6. EFA gains distribution, maintainers, sponsors, Pro interest, and consulting leads.
...
integration becomes another reason to install EFA, and every serious EFA user
