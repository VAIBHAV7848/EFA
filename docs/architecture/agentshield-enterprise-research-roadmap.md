...
- EFA-Tools now consumes that fleet primitive in hosted security review:
...
default. EFA-Tools follow-up routing now consumes fleet summaries and surfaces
...
AgentShield is already connected conceptually to the EFA Tools GitHub App.
...
- Billing readiness checks shared with EFA-Tools so payment state never changes
...
- It aligns AgentShield with EFA's harness-agnostic positioning.
...
Connect AgentShield findings to EFA-Tools follow-up routing.

Artifacts:

- Finding fingerprints compatible with EFA-Tools issue caps.
...
- EFA-Tools can consume the finding fingerprints or backlog export without
