...
EFA accepts ideas from outside repos, but shipped skills need to become EFA-native surfaces.
...
- adapt it to EFA's current install surfaces, validation flow, and repo conventions
- remove unnecessary external branding, dependency assumptions, and upstream-specific framing

The goal is reuse without turning EFA into a thin wrapper around someone else's runtime.
...
- there is no better EFA-native name already in the repo
...
Rename the skill when EFA meaningfully expands, narrows, or repackages the original work.

Typical triggers:

- EFA adds substantial new behavior, structure, or guidance
- the original name is vendor-forward or community-brand-forward instead of workflow-forward
- the contribution overlaps an existing EFA surface and needs a clearer boundary
...
- prefer EFA-native names like `product-capability` over vague imported planning labels if the scope changed materially

## Dependency Policy

EFA prefers the narrowest native surface that gets the job done:
...
- vendor or recreate the relevant logic inside EFA when practical
...
1. Is this a real reusable surface in EFA, or just documentation for another tool?
2. Does the current name still match the EFA-shaped surface?
3. Is there already an EFA skill that owns most of this behavior?
4. Are we importing a concept, or importing someone else's product identity?
5. Would an EFA user understand the purpose of this skill without knowing the upstream repo?
