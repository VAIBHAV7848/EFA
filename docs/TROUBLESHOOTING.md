...
Community-reported workarounds for current Claude Code bugs that can affect EFA users.

These are upstream Claude Code behaviors, not EFA bugs. The entries below summarize the production-tested workarounds collected in [issue #644](https://github.com/VAIBHAV7848/EFA/issues/644) on Claude Code `v2.1.79` (macOS, heavy hook usage, MCP connectors enabled). Treat them as pragmatic stopgaps until upstream fixes land.
...
- Use EFA's `strategic-compact` guidance instead of forcing a lower auto-compact threshold.
...
- Advanced users sometimes script a local `/reload` command around `kill -HUP $PPID`, but EFA does not ship that because it is shell-dependent and not universally reliable.
...
- [hooks/README.md](../hooks/README.md) for EFA's documented hook lifecycle and exit-code behavior.
- [token-optimization.md](./token-optimization.md) for cost and context management settings.
- [issue #644](https://github.com/VAIBHAV7848/EFA/issues/644) for the original report and tested environment.
