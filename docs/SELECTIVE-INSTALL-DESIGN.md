# EFA Selective Install Design

## Purpose

This document defines the user-facing selective-install design for EFA.
...
- how users choose EFA components
- what the CLI should feel like
- what config file should exist
- how installation should behave across harness targets
- how the design maps onto the current EFA codebase without requiring a rewrite

## Problem

Today EFA still feels like a large payload installer even though the repo now
...
The selective-install system should make EFA feel composable instead of
all-or-nothing.
...
1. Let users install a small default EFA footprint quickly.
...
5. Preserve backward compatibility with the current `efa-install typescript`
   style during rollout.

## Non-Goals

- packaging EFA into multiple npm packages in the first phase
...
A user should be able to get a useful EFA install with one command:

```bash
efa install --target claude --profile core
```
...
```bash
efa install --target cursor --profile developer --with lang:typescript --with framework:nextjs --dry-run
```
...
```bash
efa install --config efa-install.json
```
...
These are the default EFA building blocks:
...
Capability packs are cross-cutting EFA feature bundles.
...
- `core`
  minimal baseline, safe default for most users trying EFA
- `developer`
...
```bash
efa install --target claude --profile developer --with lang:typescript --with framework:nextjs --without capability:orchestration
```
...
```bash
efa install [--target <target>] [--profile <name>] [--with <component>]... [--without <component>]... [--config <path>] [--dry-run] [--json]
```
...
```bash
efa install --target claude --profile core
efa install --target cursor --profile developer --with lang:typescript --with framework:nextjs
efa install --target antigravity --with capability:security --with lang:python
efa install --config efa-install.json
```
...
```bash
efa plan [same selection flags as install]
```
...
```bash
efa-install typescript
efa-install --target cursor typescript
efa typescript
```
...
```json
{
  "$schema": "./schemas/efa-install-config.schema.json",
  "version": 1,
  "target": "cursor",
  "profile": "developer",
  "include": [
...
The commands differ in action, not in how EFA understands the selected install.
...
- `efa-install.json` config file parsing
...
## Mapping To Current EFA Manifests
...
That is the shortest path from the current EFA codebase to a real selective
install experience that feels like EFA 2.0 instead of a large legacy installer.
