...
![EFA - das Harness-native Operator-System für agentische Arbeit](../../assets/hero.png)
...
EFA v2.0.0-rc.1 ergänzt diese wiederverwendbare Schicht um die öffentliche Hermes-Operator-Story: Beginne mit dem [Hermes-Setup-Leitfaden](../../docs/HERMES-SETUP.md), prüfe anschließend die [rc.1-Release-Notes](../../docs/releases/2.0.0-rc.1/release-notes.md) und die [Cross-Harness-Architektur](../../docs/architecture/cross-harness.md).
...
<sub>**OSS bleibt kostenlos.** Dieses Repo ist für immer MIT-lizenziert. EFA Pro ist die gehostete GitHub App für private Repos. <a href="https://github.com/sponsors/VAIBHAV7848">Sponsoren</a> und <a href="https://efa.tools/pricing">Pro-Abonnenten</a> finanzieren die Arbeit — deshalb liefert ein einzelner Maintainer wöchentlich über 7 Harnesses hinweg aus.</sub>
...
<tr>
<td width="33%">
<a href="https://x.com/VAIBHAV7848/status/2012378465664745795">
<img src="../../assets/images/guides/shorthand-guide.png" alt="The Shorthand Guide to Everything For Ai" />
</a>
</td>
<td width="33%">
<a href="https://x.com/VAIBHAV7848/status/2014040193557471352">
<img src="../../assets/images/guides/longform-guide.png" alt="The Longform Guide to Everything For Ai" />
</a>
</td>
<td width="33%">
<a href="https://x.com/VAIBHAV7848/status/2033263813387223421">
...
### v2.0.0-rc.1 — Oberflächen-Refresh, Operator-Workflows und EFA 2.0 Alpha (April 2026)

- **Dashboard-GUI** — Neue Tkinter-basierte Desktop-Anwendung (`efa_dashboard.py` oder `npm run dashboard`) mit Umschalter für dunkles/helles Theme, Schriftanpassung und Projektlogo in Kopfzeile und Taskleiste.
...
- **Erweiterung von Operator- und Outbound-Workflows** — `brand-voice`, `social-graph-ranker`, `connections-optimizer`, `customer-billing-ops`, `efa-tools-cost-audit`, `google-workspace-ops`, `project-flow-ops` und `workspace-surface-audit` runden die Operator-Spur ab.
...
- **EFA 2.0 Alpha ist im Tree** — der Rust-Control-Plane-Prototyp in `efa2/` baut jetzt lokal und stellt die Befehle `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` und `daemon` bereit. Er ist als Alpha nutzbar, aber noch kein allgemeines Release.
- **Operator-Status-Snapshots** — `efa status --markdown --write status.md` verwandelt den lokalen State Store in eine portable Übergabe, die Bereitschaft, aktive Sessions, Skill-Run-Gesundheit, Install-Gesundheit, ausstehende Governance-Events und verknüpfte Arbeitselemente aus Linear/GitHub/Handovers abdeckt. Nutze `efa work-items upsert ...` für manuelle Einträge, `efa work-items sync-github --repo owner/repo` für den Queue-Status von PRs/Issues und `efa status --exit-code`, um Automatisierung scheitern zu lassen, wenn die Bereitschaft Aufmerksamkeit erfordert.
- **Ökosystem-Härtung** — AgentShield, EFA-Tools-Kostenkontrollen, Arbeiten am Billing-Portal und Website-Refreshes werden weiterhin rund um das Kern-Plugin ausgeliefert, statt in separate Silos abzudriften.
...
- **Harness-First-Release** — EFA ist nun ausdrücklich als Performance-System für Agent-Harnesses positioniert, nicht nur als Config-Paket.
...
- **Hook-Laufzeitsteuerung** — `EFA_HOOK_PROFILE=minimal|standard|strict` und `EFA_DISABLED_HOOKS=...` für Laufzeit-Gating ohne Bearbeitung von Hook-Dateien.
...
- **GitHub Marketplace** — EFA-Tools-GitHub-App live unter [github.com/marketplace/efa-tools](https://github.com/marketplace/efa-tools) mit Free-/Pro-/Enterprise-Stufen
...
- **Inhaltsverlust beim Instinct-Import behoben** — `parse_instinct_file()` verwarf während `/instinct-import` stillschweigend sämtlichen Inhalt nach dem Frontmatter (Abschnitte Action, Evidence, Examples). ([#148](https://github.com/VAIBHAV7848/EFA/issues/148), [#161](https://github.com/VAIBHAV7848/EFA/pull/161))

### v1.4.0 — Mehrsprachige Rules, Installationsassistent & PM2 (Februar 2026)

- **Interaktiver Installationsassistent** — Der neue `configure-efa`-Skill bietet ein geführtes Setup mit Merge-/Überschreiben-Erkennung
...
Den vollständigen Changelog findest du unter [Releases](https://github.com/VAIBHAV7848/EFA/releases).
...
- **Stapele Installationsmethoden nicht.** Das häufigste kaputte Setup ist: zuerst `/plugin install`, danach `install.sh --profile full` oder `npx efa-install --profile full`.

Falls du bereits mehrere Installationen übereinandergelegt hast und Dinge doppelt aussehen, springe direkt zu [EFA zurücksetzen / deinstallieren](#efa-zurücksetzen--deinstallieren).
...
Falls du nicht sicher bist, welches EFA-Profil oder welche Komponente du installieren sollst, frage den mitgelieferten Advisor aus jedem beliebigen Projekt:
...
EFA hat jetzt drei öffentliche Bezeichner, und sie sind nicht austauschbar:

- GitHub-Quell-Repo: `VAIBHAV7848/EFA`
- Claude-Marketplace-/Plugin-Bezeichner: `efa@efa`
- npm-Paket: `efa-universal`

Das ist beabsichtigt. Anthropic-Marketplace-/Plugin-Installationen werden über einen kanonischen Plugin-Bezeichner gekeyt, daher verwendet EFA `efa@efa`, um Tool-Namen und Slash-Command-Namespaces kurz genug für strenge Desktop-/API-Validatoren zu halten. Ältere Beiträge zeigen möglicherweise noch den früheren langen Marketplace-Bezeichner; behandle diesen lediglich als Legacy-Alias. Das npm-Paket blieb davon getrennt bei `efa-universal`, daher verwenden npm-Installationen und Marketplace-Installationen absichtlich unterschiedliche Namen.
...
>
> Falls du EFA bereits über `/plugin install` installiert hast, **führe danach nicht `./install.sh --profile full`, `.\install.ps1 --profile full` oder `npx efa-install --profile full` aus**. Das Plugin lädt EFA-Skills, -Commands und -Hooks bereits. Wird der vollständige Installer nach einer Plugin-Installation ausgeführt, kopiert er dieselben Oberflächen in deine Benutzerverzeichnisse und kann doppelte Skills sowie doppeltes Laufzeitverhalten erzeugen.
>
> Kopiere für Plugin-Installationen manuell nur die `rules/`-Verzeichnisse, die du willst, nach `~/.claude/rules/efa/`. Beginne mit `rules/common` plus einem Sprach- oder Framework-Paket, das du tatsächlich verwendest. Kopiere nicht jedes Rules-Verzeichnis, es sei denn, du willst diesen gesamten Kontext ausdrücklich in Claude haben.
>
> Verwende den vollständigen Installer nur dann, wenn du eine vollständig manuelle EFA-Installation statt des Plugin-Pfads durchführst.
>
> Falls dein lokales Claude-Setup gelöscht oder zurückgesetzt wurde, bedeutet das nicht, dass du EFA erneut kaufen musst. Beginne mit `node scripts/efa.js list-installed`, führe dann `node scripts/efa.js doctor` und `node scripts/efa.js repair` aus, bevor du irgendetwas neu installierst. Das stellt EFA-verwaltete Dateien üblicherweise wieder her, ohne dein Setup neu aufzubauen. Falls das Problem im Konto- oder Marketplace-Zugriff für EFA Tools liegt, behandle die Konto-/Abrechnungswiederherstellung separat.
...
# Plugin-Installationspfad: nur EFA-Rules in einen EFA-eigenen Namespace kopieren
mkdir -p ~/.claude/rules/efa
cp -R rules/common ~/.claude/rules/efa/
cp -R rules/typescript ~/.claude/rules/efa/

# Vollständig manueller EFA-Installationspfad (nutze diesen statt /plugin install)
...
### EFA zurücksetzen / deinstallieren

Falls sich EFA doppelt, aufdringlich oder kaputt anfühlt, installiere es nicht weiter über sich selbst.

- **Plugin-Pfad:** Entferne das Plugin aus Claude Code, lösche dann die konkreten Rule-Ordner, die du manuell unter `~/.claude/rules/efa/` kopiert hast.
...
Entferne anschließend EFA-verwaltete Dateien:
...
EFA entfernt nur Dateien, die in seinem Install-State erfasst sind. Es löscht keine fremden Dateien, die es nicht installiert hat.

Falls du Methoden gestapelt hast, räume in dieser Reihenfolge auf:

1. Entferne die Claude-Code-Plugin-Installation.
2. Führe den EFA-Uninstall-Befehl vom Repo-Root aus, um über den Install-State verwaltete Dateien zu entfernen.
...
# Bestehende Slash-artige Command-Namen funktionieren weiterhin, während EFA von commands/ wegmigriert.

# Die Plugin-Installation verwendet die kanonische Namespace-Form
/efa:plan "Benutzerauthentifizierung hinzufügen"
...
Starte das Desktop-Dashboard, um EFA-Komponenten visuell zu erkunden:
...
|   |-- configure-efa/              # Interaktiver Installationsassistent (NEU)
...
|
|-- rules/            # Stets zu befolgende Richtlinien (nach ~/.claude/rules/efa/ kopieren)
...
[GitHub App installieren](https://github.com/apps/skill-creator) | [efa.tools](https://efa.tools)
...
Verwende `/security-scan` in Claude Code, um es auszuführen, oder füge es per [GitHub Action](https://github.com/VAIBHAV7848/agentshield) zur CI hinzu.
...
**Historie:** Dies hat in diesem Repo wiederholte Fix-/Revert-Zyklen verursacht ([#29](https://github.com/VAIBHAV7848/EFA/issues/29), [#52](https://github.com/VAIBHAV7848/EFA/issues/52), [#103](https://github.com/VAIBHAV7848/EFA/issues/103)). Das Verhalten änderte sich zwischen Claude-Code-Versionen, was zu Verwirrung führte. Wir haben jetzt einen Regressionstest, der verhindert, dass dies erneut eingeführt wird.
...
> mkdir -p ~/.claude/rules/efa
> cp -r rules/common ~/.claude/rules/efa/
> cp -r rules/typescript ~/.claude/rules/efa/   # wähle deinen Stack
...
# Manuelle Installationen nicht unter ~/.claude/skills/efa/ verschachteln.
...
Falls du EFA über `/plugin install` installiert hast, kopiere diese Hooks nicht in `settings.json`. Claude Code v2.1+ lädt Plugin-`hooks/hooks.json` bereits automatisch, und sie in `settings.json` zu duplizieren, verursacht doppelte Ausführung und Cross-Platform-Hook-Konflikte.

Windows-Hinweis: Das Claude-Konfigurationsverzeichnis ist `%USERPROFILE%\\.claude`, nicht `~/claude`.

#### MCPs konfigurieren

Claude-Plugin-Installationen aktivieren die mitgelieferten MCP-Server-Definitionen von EFA absichtlich nicht automatisch. Das vermeidet überlange Plugin-MCP-Tool-Namen auf strengen Drittanbieter-Gateways und hält gleichzeitig das manuelle MCP-Setup verfügbar.
...
Falls du bereits eigene Kopien der von EFA mitgelieferten MCPs betreibst, setze:

```bash
export EFA_DISABLED_MCPS="github,context7,exa,playwright,sequential-thinking,memory"
```

EFA-verwaltete Install- und Codex-Sync-Flows überspringen oder entfernen diese mitgelieferten Server, statt Duplikate erneut hinzuzufügen. `EFA_DISABLED_MCPS` ist ein EFA-Install-/Sync-Filter, kein Live-Toggle für Claude Code.
...
|--------------|-----------------|------------|
| Ein neues Feature planen | `/efa:plan "Add auth"` | planner |
| Systemarchitektur entwerfen | `/efa:plan` + architect-Agent | architect |
...
Das ist das häufigste Problem. **Füge KEIN `"hooks"`-Feld zu `.claude-plugin/plugin.json` hinzu.** Claude Code v2.1+ lädt `hooks/hooks.json` aus installierten Plugins automatisch. Es explizit zu deklarieren, verursacht Fehler durch Duplikaterkennung. Siehe [#29](https://github.com/VAIBHAV7848/EFA/issues/29), [#52](https://github.com/VAIBHAV7848/EFA/issues/52), [#103](https://github.com/VAIBHAV7848/EFA/issues/103).
</details>

<details>
<summary><b>Kann ich EFA mit Claude Code an einem benutzerdefinierten API-Endpoint oder Modell-Gateway verwenden?</b></summary>

Ja. EFA hat keine Anthropic-gehosteten Transporteinstellungen fest verdrahtet. Es läuft lokal über die normale CLI-/Plugin-Oberfläche von Claude Code, daher funktioniert es mit:
...
Falls dein Gateway Modellnamen umbildet, konfiguriere das in Claude Code statt in EFA. ECCs Hooks, Skills, Commands und Rules sind modellanbieter-agnostisch, sobald die `claude`-CLI bereits funktioniert.
...
- **Codex**: Erstklassige Unterstützung sowohl für die macOS-App als auch die CLI, mit Adapter-Drift-Guards und SessionStart-Fallback. Siehe PR [#257](https://github.com/VAIBHAV7848/EFA/pull/257).
...
Diese werden nicht mit EFA mitgeliefert und nicht von diesem Repo auditiert, aber sie sind wissenswert, falls du das breitere Claude-Code-Skills-Ökosystem erkundest:
...
EFA bietet Cursor-IDE-Unterstützung mit Hooks, Rules, Agents, Skills, Commands und MCP-Konfigurationen, die an Cursors Projektlayout angepasst sind.
...
| Agents | 48 | `.cursor/agents/efa-*.md` bei Installation; präfixiert, um Kollisionen mit Benutzer- oder Marketplace-Agents zu vermeiden |
...
EFA installiert keine Root-`AGENTS.md` in `.cursor/`. Cursor behandelt verschachtelte `AGENTS.md`-Dateien als Verzeichniskontext, daher würde das Kopieren von ECCs Repo-Identität in ein Host-Projekt dieses Projekt verunreinigen.

Das Cursor-native Ladeverhalten kann je nach Cursor-Build variieren. EFA installiert Agents als `.cursor/agents/efa-*.md`; falls dein Cursor-Build keine Projekt-Agents bereitstellt, funktionieren diese Dateien weiterhin als explizite Referenzdefinitionen statt als versteckter globaler Prompt-Kontext.
...
EFA bietet **erstklassige Codex-Unterstützung** sowohl für die macOS-App als auch die CLI, mit einer Referenzkonfiguration, einem Codex-spezifischen AGENTS.md-Zusatz und gemeinsam genutzten Skills.
...
# Automatisches Setup: EFA-Assets (AGENTS.md, Skills, MCP-Server) nach ~/.codex synchronisieren
npm install && bash scripts/sync-efa-to-codex.sh
# oder: pnpm install && bash scripts/sync-efa-to-codex.sh
# oder: yarn install && bash scripts/sync-efa-to-codex.sh
# oder: bun install && bash scripts/sync-efa-to-codex.sh

# Oder manuell: die Referenzkonfiguration in dein Home-Verzeichnis kopieren
cp .codex/config.toml ~/.codex/config.toml
```

Das Sync-Skript merged EFA-MCP-Server sicher in deine bestehende `~/.codex/config.toml` mit einer **add-only**-Strategie — es entfernt oder verändert deine bestehenden Server nie. Führe es mit `--dry-run` aus, um Änderungen in der Vorschau zu sehen, oder mit `--update-mcp`, um ein erzwungenes Refresh der EFA-Server auf die neueste empfohlene Konfiguration zu erzwingen.

Für Context7 verwendet EFA den kanonischen Codex-Abschnittsnamen `[mcp_servers.context7]`, startet aber weiterhin das Paket `@upstash/context7-mcp`. Falls du bereits einen veralteten `[mcp_servers.context7-mcp]`-Eintrag hast, migriert `--update-mcp` ihn auf den kanonischen Abschnittsnamen.
...
| everything-for-ai | Entwicklungskonventionen und -Patterns für das Projekt |
...
Codex bietet **noch keine Claude-artige Parität bei der Hook-Ausführung**. Die EFA-Durchsetzung dort ist instruction-basiert über `AGENTS.md`, optionale `model_instructions_file`-Overrides sowie Sandbox-/Approval-Einstellungen.
...
EFA liefert drei Beispiel-Rollenkonfigurationen aus:
...
EFA bietet Zed-Projektunterstützung über einen konservativen `.zed`-Adapter für projektlokale Einstellungen, abgeflachte Rules, Agents, Commands und Skills.
...
Der Adapter schreibt EFA-verwaltete Dateien unter `.zed/` und hält BYOK-/OpenRouter-Credentials aus dem Repo heraus. Konfiguriere das Zed-Konto oder API-Keys über Zeds eigene Einstellungs-UI oder deine lokalen Benutzereinstellungen.

---

## OpenCode-Unterstützung

EFA bietet **vollständige OpenCode-Unterstützung** einschließlich Plugins und Hooks.
...
Für das vollständige EFA-OpenCode-Setup entweder:
...
EFA bietet **GitHub-Copilot-Unterstützung** für VS Code über das native Instruction- und Prompt-Datei-System von Copilot Chat — kein zusätzliches Tooling erforderlich.
...
- **`.github/prompts/*.prompt.md`** — wiederverwendbare Prompt-Dateien, die Nutzer bei Bedarf aufrufen. Jeder Prompt führt Copilot durch einen bestimmten EFA-Workflow wie Planung, TDD, Security-Review, Build-Fix oder Refactor.
...
| EFA-Feature | Copilot-Entsprechung |
|-------------|-------------------|
...
GitHub Copilot hat kein Hook-System und keine Subagent-API, daher sind ECCs Hook-Automatisierungen (Auto-Formatierung, TypeScript-Prüfung, Session-Persistenz, Dev-Server-Guard) sowie die Agent-Delegation nicht verfügbar. Die Instruction- und Prompt-Schicht bringt dennoch die vollständige EFA-Coding-Philosophie — Standards, Sicherheit, TDD und Workflow — in jede Copilot-Chat-Session.

---

## Cross-Tool-Feature-Parität

EFA ist das **erste Plugin, das jedes große KI-Coding-Tool ausreizt**. So vergleicht sich jeder Harness:
...
| `EFA_CONTEXT_MONITOR_COST_WARNINGS` | on | **off für Abonnement-Nutzer** | Unterdrückt agentenseitige API-Rate-Schätzwarnungen, behält aber Kontext-/Scope-/Loop-Warnungen |
...
Falls du ein Claude-Abonnement nutzt und die API-Rate-Schätzungen des Kontext-Monitors nicht nützlich sind, setze `EFA_CONTEXT_MONITOR_COST_WARNINGS=off`. Das unterdrückt nur die agentenseitigen Kostenwarnungen; es deaktiviert keine Warnungen zu Kontexterschöpfung, Scope oder Loops.
...
- Verwende `EFA_DISABLED_MCPS` nur, um EFA-generierte MCP-Konfigurationen während der Install-/Sync-Flows zu filtern
...
Projekte, die auf EFA aufbauen oder davon inspiriert sind:
...
Etwas mit EFA gebaut? Öffne einen PR, um es hier hinzuzufügen.
...
[**Sponsor werden**](https://github.com/sponsors/VAIBHAV7848) | [Sponsor-Stufen](../../SPONSORS.md) | [Sponsoring-Programm](../../SPONSORING.md)

---

## Star-Verlauf

[![Star History Chart](https://api.star-history.com/svg?repos=VAIBHAV7848/EFA&type=Date)](https://star-history.com/#VAIBHAV7848/EFA&Date)

---

## Links

- **Kurzleitfaden (Hier starten):** [The Shorthand Guide to Everything For Ai](https://x.com/VAIBHAV7848/status/2012378465664745795)
- **Langleitfaden (fortgeschritten):** [The Longform Guide to Everything For Ai](https://x.com/VAIBHAV7848/status/2014040193557471352)
- **Security-Leitfaden:** [Security-Leitfaden](../../the-security-guide.md) | [Thread](https://x.com/VAIBHAV7848/status/2033263813387223421)
- **Folgen:** [@VAIBHAV7848](https://x.com/VAIBHAV7848)
