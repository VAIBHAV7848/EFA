# EFA 2.0 Session Adapter Discovery

## Purpose

This document turns the March 11 EFA 2.0 control-plane direction into a
...
- `scripts/lib/session-adapters/canonical-session.js`
  defines the canonical `efa.session.v1` normalization layer
...
In practice, EFA can already answer:
...
## Gap Between EFA 1.x And EFA 2.0

EFA 1.x currently has two different "session" surfaces:
...
The missing EFA 2.0 layer is a harness-neutral session adapter boundary that
can normalize:
...
EFA 2.0 should introduce a canonical session adapter contract.
...
This provides a non-orchestrated baseline for EFA 2.0.
...
3. Should event streaming be part of v1, or should EFA 2.0 ship snapshot-only
   first?
...
5. Should the adapter registry live inside this repo long-term, or move into the
   eventual EFA 2.0 control-plane app once the interface stabilizes?
...
The shortest path to EFA 2.0 is:
