...
Hermes is the operator shell. EFA is the reusable system behind it.
...
- EFA skills, agents, commands, hooks, and MCP configs from this repo
...
Use Hermes as the front door and EFA as the reusable workflow substrate.
...
Telegram / CLI / TUI
      ...
      Hermes
        ↓
 EFA skills + hooks + MCPs + generated workflow packs
        ↓
...
- `~/.hermes/config.yaml`
  ...
  - plugin loading
- `~/.hermes/skills/efa-imports/`
  - EFA skills copied in for Hermes-native use
...
- EFA for skills, rules, prompts, and cross-harness conventions
...
0. Run `efa migrate audit --source ~/.hermes` first to inventory the legacy workspace and see which parts already map onto EFA2.
0.5. Plan and scaffold migration artifacts before importing anything:
   - generate reviewable plans with `efa migrate plan` and `efa migrate scaffold`
   - scaffold reusable legacy skills with `efa migrate import-skills --output-dir migration-artifacts/skills`
   - scaffold tool translation templates with `efa migrate import-tools --output-dir migration-artifacts/tools`
   - scaffold bridge plugin templates with `efa migrate import-plugins --output-dir migration-artifacts/plugins`
   - preview recurring jobs with `efa migrate import-schedules --dry-run`
   - preview gateway dispatch with `efa migrate import-remote --dry-run`
   - preview safe env/service context with `efa migrate import-env --dry-run`
   - import sanitized workspace memory with `efa migrate import-memory`
1. Install EFA and verify the baseline harness setup with `node tests/run-all.js`; the expected result is a zero-failure test summary.
2. Install Hermes and point it at EFA-imported skills.
...
EFA v2.0.0-rc.1 documents the Hermes surface and ships launch collateral now.
