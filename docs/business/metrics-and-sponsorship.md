...
```bash
# Weekly downloads
curl -s https://api.npmjs.org/downloads/point/last-week/efa-universal
curl -s https://api.npmjs.org/downloads/point/last-week/efa-agentshield

# Last 30 days
curl -s https://api.npmjs.org/downloads/point/last-month/efa-universal
curl -s https://api.npmjs.org/downloads/point/last-month/efa-agentshield
```

### GitHub repository adoption

```bash
gh api repos/VAIBHAV7848/EFA \
...
```bash
gh api repos/VAIBHAV7848/EFA/traffic/views
gh api repos/VAIBHAV7848/EFA/traffic/clones
```
...
- [EFA Tools Marketplace](https://github.com/marketplace/efa-tools)
...
> EFA is now positioned as an agent harness performance system, not a config repo.
