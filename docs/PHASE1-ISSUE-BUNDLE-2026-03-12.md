...
- `#423` Implement manifest-driven selective install profiles for EFA
- `#421` Add EFA install-state plus uninstall / doctor / repair lifecycle
- `#424` Define canonical session adapter contract for EFA 2.0 control plane
...
Implement manifest-driven selective install profiles for EFA
...
EFA still installs primarily by target and language. The repo now has first-pass
...
Implement manifest-driven install execution for current EFA targets:
...
- `efa-install --profile <name>`
- `efa-install --modules <id,id,...>`
...
Add EFA install-state plus uninstall / doctor / repair lifecycle
...
EFA has no canonical installed-state record. That makes uninstall, repair, and
...
- what paths EFA owns
- how to remove or repair only EFA-managed files
...
- Claude: `~/.claude/efa/install-state.json`
- Cursor: `./.cursor/efa-install-state.json`
- Antigravity: `./.agent/efa-install-state.json`
...
- `uninstall` removes only EFA-managed files and leaves unrelated local files
  alone
...
Define canonical session adapter contract for EFA 2.0 control plane
...
EFA now has real orchestration/session substrate, but it is still
...
Without that adapter contract, any future EFA 2.0 operator shell will be forced
...
- Full EFA 2.0 UI in the same issue
...
EFA now has a large and growing skill surface, but generated/imported/learned
...
As EFA grows, the repo needs explicit rules for where generated skill artifacts
