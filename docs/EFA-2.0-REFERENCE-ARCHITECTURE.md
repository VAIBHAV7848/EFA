# EFA 2.0 Reference Architecture

Current execution mirror:
[`EFA-2.0-GA-ROADMAP.md`](EFA-2.0-GA-ROADMAP.md).

This document turns the May 2026 reference sweep into concrete EFA backlog
...
| Reference | Primary pressure on EFA 2.0 | Concrete EFA delta |
| --- | --- | --- |
...
| [`jarrodwatts/claude-hud`](https://github.com/jarrodwatts/claude-hud) | Always-visible Claude Code statusline for context, tools, agents, todos, and transcript-backed activity. | Formalize the EFA HUD/status payload for context, cost, tool calls, active agents, todos, queue state, checks, and risk. |
| [`stanford-iris-lab/meta-harness`](https://github.com/stanford-iris-lab/meta-harness) | Automated search over task-specific harness design: what to store, retrieve, and show. | Split EFA improvement loops into scenario spec, proposer trace, verifier result, and promoted playbook. |
...
| [`NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent) | Self-improving operator shell with memories, skills, scheduler, gateways, subagents, terminal backends, and migration tooling. | Keep EFA portable across local, SSH, container, and hosted terminal backends without hiding the underlying commands. |
...
EFA 2.0 should be a harness operating system, not only a catalog of commands,
agents, and skills.
...
│ AgentShield policies/SARIF, EFA Tools checks, billing,       │
...
- `everything-for-ai`: extend the adapter compliance matrix and public
  scorecard onramp.
- `efa2`: surface session/worktree state through a stable local payload before
  adding hosted telemetry.
- `EFA-Tools`: consume the same lifecycle events for PR checks, issue routing,
  and Linear sync.
...
- Keep `docs/architecture/observability-readiness.md` as the operator-facing
  readiness gate.
- Define a versioned HUD/status JSON contract that both EFA2 and EFA Tools can
  consume.
...
- Use RAG/reference sets for vetted EFA patterns, team history, CI failures,
  review outcomes, harness config quality, and security decisions.

Repo work:

- `everything-for-ai`: document scenario specs, verifier contracts, and
  playbook promotion rules.
- `EFA-Tools`: map analyzer findings to PR comments, check runs, and Linear
...
### EFA Tools Commercial And Review Platform

EFA Tools should become the GitHub-native layer for billing, deep analysis,
...
- Deep analyzer comparable in scope to the useful parts of GitGuardian,
  ...
  and harness drift.
- RAG/reference set over vetted EFA patterns, historical PR outcomes,
...
5. Audit EFA Tools billing and check-run surfaces.
