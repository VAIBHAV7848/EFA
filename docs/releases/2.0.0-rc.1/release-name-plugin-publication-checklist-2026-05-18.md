# EFA v2.0.0-rc.1 Release Name And Plugin Publication Checklist

Snapshot date: 2026-05-18. Canonical repo decision refreshed 2026-05-19
after the public repo rename to `VAIBHAV7848/EFA`; release/package state refreshed
...
- Keep the GitHub repo at `VAIBHAV7848/EFA`.
- Keep the npm package as `efa-universal`.
- Keep Claude and Codex plugin slugs as `efa`.
- Publish the npm prerelease on the `next` dist-tag, not `latest`.
- Do not rename the npm package to `efa` or `@VAIBHAV7848/EFA` before rc.1.
- Treat `VAIBHAV7848/EFA` as the canonical public repo for rc.1 and GA release
  copy.

Reasons:

- `efa-universal` is the current working install and package surface.
- `efa` on npm is occupied by an unrelated elliptic-curve package.
- `@VAIBHAV7848/EFA` is unclaimed on npm, but would require a migration plan.
- `VAIBHAV7848/EFA` is now the live public GitHub repo.
- Claude and Codex already expose the desired short namespace as `efa`.
...
| npm package | `efa-universal@2.0.0-rc.1` local and registry next, `1.10.0` registry latest | `node -p "require('./package.json').name + '@' + require('./package.json').version"` and `npm view efa-universal name version dist-tags --json` | Local rc.1 ready; registry `next` points to `2.0.0-rc.1`; `latest` remains `1.10.0` | Keep rc.1 on `next`; do not move to `latest` before GA approval |
| Exact npm short name | `efa` | `npm view efa name version description repository.url --json` | Occupied by unrelated `efa@0.0.2` | Do not use |
| Scoped npm short name | `@VAIBHAV7848/EFA` | `npm view @VAIBHAV7848/EFA name version --json` | 404 | Candidate only after migration plan |
| Claude plugin | `efa@2.0.0-rc.1` | `claude plugin validate .claude-plugin/plugin.json`; `claude plugin validate .`; `claude plugin tag .claude-plugin --dry-run` | Validation passed on Claude Code `2.1.143`; full plugin validation has one expected root `CLAUDE.md` context warning; dry run would create `efa--v2.0.0-rc.1` | Run dry-run tag again from the final commit, then tag/push only after approval |
...
npm view @VAIBHAV7848/EFA name version --json
...
| 3 | Verify package and plugin manifests | `node tests/plugin-manifest.test.js` and `node tests/docs/efa2-release-surface.test.js` pass | Manifest or release-surface failure |
...
| 10 | Verify npm rc | `npm view efa-universal version dist-tags --json` shows rc.1 on `next` and latest still on GA/stable | rc.1 lands on `latest` or registry output is unclear |
...
- Do not announce billing, Marketplace, or native payments until EFA Tools live
...
an official Plugin Directory, but EFA has not submitted or received an official
