# EFA v2.0.0-rc.1 Publication Evidence - 2026-05-15
...
| Evidence scope | Current `main` after PR #1932, #1933, #1934, #1935, and #1936; AgentShield #86; and EFA-Tools #75 |
...
| --- | --- | --- |
| Trunk PRs/issues | `gh pr list` and `gh issue list` for `VAIBHAV7848/EFA` | 0 open PRs, 0 open issues |
| AgentShield PRs/issues | `gh pr list` and `gh issue list` for `VAIBHAV7848/agentshield` | 0 open PRs, 0 open issues |
| JARVIS PRs/issues | `gh pr list` and `gh issue list` for `VAIBHAV7848/JARVIS` | 0 open PRs, 0 open issues |
| EFA Tools PRs/issues | `env -u GITHUB_TOKEN gh pr list` and `env -u GITHUB_TOKEN gh issue list` for `EFA-Tools/EFA-Tools` | 0 open PRs, 0 open issues |
| EFA website PRs/issues | `env -u GITHUB_TOKEN gh pr list` and `env -u GITHUB_TOKEN gh issue list` for `EFA-Tools/EFA-website` | 0 open PRs, 0 open issues |
...
| Other repo discussions | GraphQL discussion count for AgentShield, JARVIS, EFA Tools, and EFA website | Discussions disabled or 0 total |
...
The EFA Tools organization is reachable with the configured GitHub host
credential. In this shell, the exported `GITHUB_TOKEN` overrides that credential
and causes false 404/403 failures for `EFA-Tools/*`. Use `env -u GITHUB_TOKEN`
for EFA Tools verification commands until that environment override is cleaned
up.
...
| EFA 2.0 Preview and Publication | `ITO-45`, `ITO-46`, `ITO-47`, `ITO-56` |
| AgentShield Enterprise Iteration | `ITO-48`, `ITO-49` |
| EFA Tools Next-Level Platform | `ITO-50`, `ITO-51`, `ITO-52`, `ITO-53`, `ITO-54`, `ITO-59` |
...
- EFA-Tools Access Evidence 2026-05-15
...
| PR #1935 | Stabilized `efa2` current-directory-mutating tests with a test-only serialized current-dir guard, preserving the Rust release-surface gate under parallel test execution |
...
| EFA-Tools PR #75 | Tightened the native GitHub payments announcement gate so public billing claims remain blocked until live Marketplace-managed test-account readback is ready |
...
| EFA-Tools merge commits | `6d00d67043e92cadc80f160bfe947115bfef33b1` |
...
| IOC scan | `node scripts/ci/scan-supply-chain-iocs.js --root <EFA-workspace> --home` passed with 229 files inspected after the no-lifecycle install refresh |
...
| Rust release-surface gate | `cd efa2 && cargo test` passed 462/462 with the existing 14 dead-code/unused warnings |
...
- explicit blockers for GitHub release, npm `next` publish, Claude plugin,
  Codex plugin, EFA Tools billing/product-readiness, and announcements.
...
| Repo marketplace | `.agents/plugins/marketplace.json` exposes `efa@2.0.0-rc.1` with `source.path: "./"` from the marketplace root |
| Local add smoke | `HOME="$(mktemp -d)" codex plugin marketplace add <local-checkout>` added marketplace `efa` and recorded the installed marketplace root as `<local-checkout>` without touching the real Codex config |
...
- npm `efa-universal@2.0.0-rc.1` is still not published to the `next` dist-tag.
...
- EFA Tools PR #73 added a fail-closed `/api/billing/readiness`
  `announcementGate` for native GitHub payments claims, and EFA Tools PR #74
