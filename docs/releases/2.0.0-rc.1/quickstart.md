...
```bash
npm install efa-universal@next
```

`next` currently resolves to `efa-universal@2.0.0-rc.1`; `latest` remains on
...
for loop status, session traces, harness audit, and EFA2 tool-risk logs.

## First Skill

Read `skills/hermes-imports/SKILL.md` first.

It shows the intended EFA 2.0 pattern:
...
- Claude Code consumes EFA through the Claude plugin and native hooks.
- Codex consumes EFA through `AGENTS.md`, `.codex-plugin/plugin.json`, and MCP reference config.
- OpenCode consumes EFA through the OpenCode package/plugin surface.
