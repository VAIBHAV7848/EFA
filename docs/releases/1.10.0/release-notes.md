# EFA v1.10.0 Release Notes

## Positioning

EFA v1.10.0 is a surface-sync and operator-lane release.
...
EFA 2.0 is **real and usable as an alpha**, but it is **not general-availability complete**.

What exists today:

- `efa2/` Rust control-plane codebase in the main repo
- `cargo build --manifest-path efa2/Cargo.toml` passes
- `efa-tui` commands currently available:
  - `dashboard`
  - `start`
  - `sessions`
  - `status`
...
- You should not describe the full EFA 2.0 roadmap as finished.
- The right framing today is **EFA 2.0 alpha / control-plane preview**, not GA.
...
- The selective install / OSS path is still the most reliable full install for teams that want the complete EFA surface.
...
4. Treat EFA 2.0 as an alpha control-plane surface until the open P0/P1 roadmap is materially burned down.
