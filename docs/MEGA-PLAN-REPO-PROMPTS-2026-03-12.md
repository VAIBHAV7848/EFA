...
- `everything-for-ai` has finished the orchestration, Codex baseline, and
  Windows CI recovery lane.
- The next open EFA Phase 1 items are:
  - review `#399`
  - convert recurring discussion pressure into tracked issues
  - define selective-install architecture
  - write the EFA 2.0 discovery doc
- `agentshield`, `EFA-website`, and `skill-creator-app` all have dirty
...
- `applications/` is not a standalone git repo. It lives inside the parent
  workspace repo at `<EFA_ROOT>`.

## Repo: `everything-for-ai`

### Prompt A — PR `#399` Review and Merge Readiness

```text
Work in: <EFA_ROOT>/everything-for-ai
...
issues. Focus on high-signal roadmap items that unblock EFA 1.x and EFA 2.0.
...
5. EFA 2.0 discovery doc / adapter contracts
...
### Prompt C — EFA 2.0 Discovery and Adapter Spec

```text
Work in: <EFA_ROOT>/everything-for-ai

Goal:
Turn the existing EFA 2.0 vision into a first concrete discovery doc focused on
...
4. Define what stays in EFA 1.x versus what belongs in EFA 2.0.
...
- distinguish EFA 1.x toolkit from EFA 2.0 control plane clearly
...
```text
Work in: <EFA_ROOT>/skill-creator-app
...
1. Assess whether the app should support:
   ...
   - EFA convention adaptation workflows
...
- keep the focus on audited skill ingestion and EFA-compatible output
...
## Repo: `EFA` Workspace (`applications/`, `knowledge/`, `tasks/`)
...
Goal:
Use the parent EFA workspace to support the mega-plan hosted/workflow lanes.
...
- keep recommendations compatible with the current multi-repo EFA setup
