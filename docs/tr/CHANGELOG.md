...
- Hermes operatör hikayesi için genel EFA 2.0 sürüm adayı yüzeyi eklendi.
- EFA, Claude Code, Codex, Cursor, OpenCode ve Gemini genelinde yeniden kullanılabilir cross-harness altyapı olarak belgelendi.
...
- EFA/Hermes sınırı için `docs/architecture/cross-harness.md` ve regresyon kapsamı eklendi.
- `efa2/` sürümlemesi bağımsız tutuldu; release engineering aksi karar vermedikçe alpha control-plane scaffold olarak kalır.

### Notlar

- Bu bir sürüm adayıdır; tam EFA 2.0 control-plane yol haritası için GA iddiası değildir.
...
- EFA 2.0 alpha control-plane binary artık `efa2/` üzerinden yerelde build ediliyor ve ilk kullanılabilir CLI/TUI yüzeyini sunuyor.
...
- Bu sürüm bir repo-yüzeyi düzeltmesi ve ekosistem senkronizasyonudur; tam EFA 2.0 yol haritasının tamamlandığı iddiası değildir.
...
- `efa-devfleet` — Çok ajanlı orkestrasyon (#505)
- `blueprint` — Çok oturumlu yapı planlaması
- `everything-for-ai` — Öz-referansiyel EFA becerisi (#335)
...
- `EFA_HOOK_PROFILE` ve `EFA_DISABLED_HOOKS` kontrolleri eklendi.
