...
|----------------|------|------|
| Almacenamiento | Global (`~/.claude/homunculus/`) | Con alcance de proyecto (`${XDG_DATA_HOME:-~/.local/share}/efa-homunculus/projects/<hash>/`) |
...
Cada proyecto obtiene un ID hash de 12 caracteres (ej. `a1b2c3d4e5f6`). Un archivo de registro en `${XDG_DATA_HOME:-~/.local/share}/efa-homunculus/projects.json` mapea IDs a nombres legibles.
