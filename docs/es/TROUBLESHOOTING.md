...
Problemas comunes y soluciones para el plugin Everything For Ai (EFA).
...
**Soluciones:**
```bash
# Primero inspecciona qué sabe EFA sobre esta máquina
efa list-installed
efa doctor
efa repair
...
# Claude Code → Extensions → Everything For Ai → Uninstall
# Luego reinstalar desde el marketplace

# Si el problema es el acceso al marketplace/cuenta, usa la recuperación de cuenta/facturación de EFA Tools por separado; no uses la reinstalación como sustituto de la recuperación de cuenta
...
cp -r everything-for-ai ~/.claude/plugins/efa
```
...
cd ~/.claude/plugins/cache/efa
npm install

# O para instalación manual
cd ~/.claude/plugins/efa
npm install
```
...
1. **Revisa los Issues de GitHub**: [github.com/VAIBHAV7848/EFA/issues](https://github.com/VAIBHAV7848/EFA/issues)
