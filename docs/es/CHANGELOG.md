...
- Añade la superficie pública del release candidate de EFA 2.0 para la historia del operador Hermes.
- Documenta EFA como el sustrato reutilizable cross-harness para Claude Code, Codex, Cursor, OpenCode y Gemini.
...
- Añadido `docs/architecture/cross-harness.md` y cobertura de regresión para el límite EFA/Hermes.
- Mantenido el versionado de `efa2/` independiente por ahora; sigue siendo un scaffold alfa del plano de control a menos que ingeniería de releases decida lo contrario.

### Notas

- Este es un release candidate, no una declaración GA para el roadmap completo del plano de control de EFA 2.0.
...
- El binario del plano de control alfa de EFA 2.0 ya compila localmente desde `efa2/` y expone la primera superficie de CLI/TUI utilizable.
...
- `cargo build --manifest-path efa2/Cargo.toml` pasa en la línea base del repositorio.
- `efa-tui` actualmente expone `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` y `daemon`.
...
- Este lanzamiento es una corrección de la superficie del repo y una sincronización del ecosistema, no una declaración de que el roadmap completo de EFA 2.0 está completo.
...
- `efa-devfleet` — orquestación multi-agente (#505)
- `blueprint` — planificación de construcción de múltiples sesiones
- `everything-for-ai` — skill autorreferencial de EFA (#335)
...
- Añadidos controles `EFA_HOOK_PROFILE` y `EFA_DISABLED_HOOKS`.
