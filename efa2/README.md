...
`efa2/` is the current Rust-based EFA 2.0 control-plane scaffold.

It is usable as an alpha for local experimentation, but it is **not** the finished EFA 2.0 product yet.
...
EFA 2.0 is the layer above individual harness installs.
...
Open issue clusters for the broader roadmap live in the main repo issue tracker under the `efa-2.0` label.
...
The alpha is missing the higher-level operator surface that defines EFA 2.0:
...
Do not market `efa2/` as done just because the scaffold builds.

The right framing is:

- EFA 2.0 alpha exists
