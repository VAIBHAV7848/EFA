#!/bin/bash
#
# EFA Trae Uninstaller
# Uninstalls Everything For Ai workflows from a Trae project.
#
# Usage:
...
do_uninstall() {
    ...
    # Manifest file path
    MANIFEST="$trae_full_path/.efa-manifest"
    ...
    echo ""
    read -p "Are you sure you want to uninstall EFA from $trae_dir? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Uninstall cancelled."
        exit 0
