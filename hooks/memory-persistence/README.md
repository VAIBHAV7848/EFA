...
These lifecycle hook definitions document EFA's memory persistence contract for Everything For Ai plugin and manual installs.
...
- `session-activity-tracker.js` records tool usage and file activity for EFA2 status and observability.
...
| `PostToolUse` | `post:session-activity-tracker` | Record tool and file activity for EFA2 metrics | no |
...
- Bound context loaded at session start with `EFA_SESSION_START_MAX_CHARS`.
- Allow opt-out with `EFA_SESSION_START_CONTEXT=off`.
- Keep lifecycle hooks profile-gated through `EFA_HOOK_PROFILE` and `EFA_DISABLED_HOOKS`.
