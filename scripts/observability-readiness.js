...
function usage() {
  console.log([
    ...
    '',
    'Deterministic EFA 2.0 observability readiness gate.',
    '',
    'Options:',
...
function buildChecks(rootDir) {
  ...
  const observabilityRust = readText(rootDir, 'efa2/src/observability/mod.rs');
  const sessionStoreRust = readText(rootDir, 'efa2/src/session/store.rs');
  const sessionManagerRust = readText(rootDir, 'efa2/src/session/manager.rs');
  ...
  const gaRoadmap = readText(rootDir, 'docs/EFA-2.0-GA-ROADMAP.md');
  ...
  const releaseSurfaceTest = readText(rootDir, 'tests/docs/efa2-release-surface.test.js');
  ...
  return [
    ...
    },
    {
      ...
      pass: fileExists(rootDir, 'docs/architecture/hud-status-session-control.md')
        ...
        && hudStatusFixture.schema_version === 'efa.hud-status.v1'
    ...
    {
      id: 'efa2-tool-risk-ledger',
      category: 'Tool Activity',
      points: 3,
      path: 'efa2/src/observability/mod.rs',
      description: 'EFA2 records tool calls with risk scoring and paginated queries',
      pass: fileExists(rootDir, 'efa2/src/observability/mod.rs')
      ...
      fix: 'Restore EFA2 tool logging, risk scoring, store queries, and metrics sync.'
    },
    {
    ...
    },
    {
      ...
      pass: fileExists(rootDir, 'docs/releases/2.0.0-rc.1/publication-readiness.md')
        ...
        && fileExists(rootDir, 'tests/docs/efa2-release-surface.test.js')
...
function buildReport(rootDir) {
  ...
  return {
    schema_version: 'efa.observability-readiness.v1',
