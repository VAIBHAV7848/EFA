...
function parsePort(v) {
  ...
  if (isNaN(n) || n < 1 || n > 65535) { console.error('[EFA] Invalid port: ' + v + ' — using 3456'); return 3456; }
  return n;
}
const PORT = parsePort(process.argv[2] || process.env.EFA_DASHBOARD_PORT || '3456');
...
<div class="header">
  <div class="brand" id="brand-link">
    ...
    <h1><span id="t-title">EFA Capabilities</span> <span class="ver">v2.0.0-rc.1</span></h1>
  </div>
...
<div class="footer">
  <a href="https://github.com/VAIBHAV7848/EFA" target="_blank">github.com/VAIBHAV7848/EFA</a>
  <span class="dt"></span>
  <span>EFA v2.0.0-rc.1</span>
  <span class="dt"></span>
  <span id="t-contribution">Contribution to EFA</span>
  <span class="dt"></span>
...
let lang = localStorage.getItem('efa-lang') || 'en';
let suggIdx = -1;
...
// Recently viewed
function recents() { try { return JSON.parse(localStorage.getItem('efa-recent') || '[]'); } catch { return []; } }
function addRecent(type, name) {
  ...
  localStorage.setItem('efa-recent', JSON.stringify(r));
}
function clearRecents() { localStorage.removeItem('efa-recent'); location.hash=''; location.reload(); }
...
// Language
function setLang(l) {
  lang = l; localStorage.setItem('efa-lang', l);
...
function toggleTheme() {
  ...
  localStorage.setItem('efa-theme', h.dataset.theme);
}
if (localStorage.getItem('efa-theme')) document.documentElement.dataset.theme = localStorage.getItem('efa-theme');
...
if (require.main === module) {
  server.listen(PORT, () => {
    console.log(`\n    EFA Capabilities  →  http://localhost:${PORT}\n`);
