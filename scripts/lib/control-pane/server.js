...
function usage() {
  return [
    'Usage:',
    '  node scripts/control-pane.js [--host 127.0.0.1] [--port 8765] [--db <efa2.db>] [--state-db <state.db>] [--config <efa2.toml>] [--query <text>]',
    '',
    'Options:',
    '  --state-db <path>  Read agent work items from an EFA state-store database',
...
function createControlPaneServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && requestUrl.pathname === '/assets/efa-icon.svg') {
        const iconPath = path.join(repoRoot, 'assets', 'efa-icon.svg');
