...
const SNAPSHOT_SCHEMA_VERSION = 'efa.control-pane.snapshot.v1';
const DEFAULT_STATE_STORE_RELATIVE_PATH = path.join('.claude', 'efa', 'state.db');
...
function defaultDbPath(env = process.env) {
  return path.join(homeDir(env), '.claude', 'efa2.db');
}
...
function defaultConfigPaths(cwd = process.cwd(), env = process.env) {
  ...
  const paths = [
    path.join(home, 'Library', 'Application Support', 'efa2', 'config.toml'),
    path.join(home, '.config', 'efa2', 'config.toml'),
    path.join(home, '.claude', 'efa2.toml'),
  ];

  let current = path.resolve(cwd);
  while (current && current !== path.dirname(current)) {
    paths.push(path.join(current, '.claude', 'efa2.toml'));
    paths.push(path.join(current, 'efa2.toml'));
...
function resolveControlPaneConfig(options = {}) {
  ...
  return {
    ...normalizeConfig(merged, {
      env,
      dbPath: options.dbPath || env.EFA2_DB_PATH || null,
      stateDbPath: options.stateDbPath || env.EFA_STATE_DB_PATH || null,
    }),
