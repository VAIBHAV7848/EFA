...
const SESSION_SCHEMA_VERSION = 'efa.session.v1';
const SESSION_RECORDING_SCHEMA_VERSION = 'efa.session.recording.v1';
const DEFAULT_RECORDING_DIR = path.join(os.tmpdir(), 'efa-session-recordings');
...
function resolveRecordingDir(options = {}) {
  ...
  if (typeof process.env.EFA_SESSION_RECORDING_DIR === 'string' && process.env.EFA_SESSION_RECORDING_DIR.length > 0) {
    return path.resolve(process.env.EFA_SESSION_RECORDING_DIR);
  }
