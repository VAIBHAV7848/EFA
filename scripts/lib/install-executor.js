...
const CLAUDE_EFA_NAMESPACE = 'efa';
...
function addCursorAgentDataScaffoldOperations(operations, options) {
  ...
  addFileCopyOperation(operations, {
    ...
    sourceRelativePath: path.join('scaffolds', 'cursor', 'efa-agent-data.json'),
    destinationPath: path.join(options.targetRoot, 'efa-agent-data.json'),
  ...
  addFileCopyOperation(operations, {
    moduleId: options.moduleId,
    sourceRoot: options.sourceRoot,
    sourceRelativePath: path.join('scaffolds', 'cursor', 'rules', 'efa-agent-data-home.mdc'),
    destinationPath: path.join(options.targetRoot, 'rules', 'efa-agent-data-home.mdc'),
    strategy: 'preserve-relative-path',
  });
...
function planClaudeStyleLegacyInstall(context, { adapterId, adapterRootInput, rulesDir: rulesDirOverride }) {
  ...
  const rulesDir = rulesDirOverride || path.join(targetRoot, 'rules', CLAUDE_EFA_NAMESPACE);
