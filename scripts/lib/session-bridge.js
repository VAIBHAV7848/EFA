...
/**
 * Shared session bridge utilities for EFA hooks.
 *
...
function getBridgePath(sessionId) {
  return path.join(os.tmpdir(), `efa-metrics-${sessionId}.json`);
}
 ...
 * writers (e.g. PostToolUse `efa-metrics-bridge` and the background
 * `efa-statusline`, both writing to the same session bridge) do not
 ...
 * `scripts/hooks/efa-metrics-bridge.js` (commit 9b1d8918) for the
...
function resolveSessionId() {
  const raw = process.env.EFA_SESSION_ID || process.env.CLAUDE_SESSION_ID || '';
