 ...
 *
 * Controls:
 * - EFA_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - EFA_DISABLED_HOOKS=comma,separated,hook,ids
 */
...
function getHookProfile() {
  const raw = String(process.env.EFA_HOOK_PROFILE || 'standard').trim().toLowerCase();
  return VALID_PROFILES.has(raw) ? raw : 'standard';
}

function getDisabledHookIds() {
  const raw = String(process.env.EFA_DISABLED_HOOKS || '');
...
function isDryRun() {
  return process.env.EFA_DRY_RUN === '1';
}
