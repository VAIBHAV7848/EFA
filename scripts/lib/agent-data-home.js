/**
 * Resolve EFA agent data home (memory persistence root) across harnesses.
 *
 ...
 * purpose, consistent with EFA script modules elsewhere. Automated PR reviewers
 ...
 *
 * @see https://github.com/VAIBHAV7848/EFA/issues/2065
 */

const fs = require('fs');
const path = require('path');

const AGENT_DATA_HOME_ENV = 'EFA_AGENT_DATA_HOME';
const DEFAULT_CLAUDE_DIR_NAME = '.claude';
const DEFAULT_CURSOR_EFA_DIR_SEGMENTS = ['.cursor', 'efa'];
const PROJECT_CONFIG_RELATIVE = path.join('.cursor', 'efa-agent-data.json');
 ...
 * dependency and risk divergent defaults for `~/.cursor/efa` vs `~/.claude`.
 *
...
/**
 * Project root for a config file under .cursor/efa-agent-data.json.
 */
...
function getDefaultCursorAgentDataHome() {
  return path.join(getHomeDirFromEnv(), ...DEFAULT_CURSOR_EFA_DIR_SEGMENTS);
}
...
function readProjectConfigAt(configPath) {
  ...
  try {
    ...
    const candidate = parsed.agentDataHome || parsed.EFA_AGENT_DATA_HOME;
    if (typeof candidate !== 'string' || !candidate.trim()) return null;
    const projectRoot = resolveProjectRootFromConfigPath(configPath);
    return expandHomePath(candidate, projectRoot);
  } catch (error) {
    console.error(
      `[EFA] Failed to read or parse agent data config at ${configPath}: ${error.message}`
    );
    return null;
  }
}
...
/**
 * Set EFA_AGENT_DATA_HOME on the current process when unset (hook subprocess safety net).
...
function getCursorSessionEnvPayload(options = {}) {
  ...
  return {
    EFA_AGENT_DATA_HOME: agentDataHome,
  };
}
...
module.exports = {
  ...
  DEFAULT_CURSOR_EFA_DIR_SEGMENTS,
