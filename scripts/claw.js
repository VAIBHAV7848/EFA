#!/usr/bin/env node
/**
 * NanoClaw v2 — Barebones Agent REPL for Everything For Ai
 *
...
function loadEFAContext(skillList) {
...
function main() {
  ...
  let efaContext = loadEFAContext(state.skills);
  ...
  const prompt = () => {
    rl.question('claw> ', (input) => {
      ...
      if (line.startsWith('/load ')) {
        ...
        efaContext = loadEFAContext(state.skills);
      ...
      const response = askClaude(efaContext, history, line, state.model);
      console.log(`\n${response}\n`);
