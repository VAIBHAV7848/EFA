...
const MAX_LIMIT = 20;
const SCHEMA_VERSION = 'efa.consult.v1';
...
function showHelp(exitCode = 0) {
  console.log(`
Consult EFA install components and profiles from any project
...
function commandFor(kind, id, target) {
  if (kind === 'profile') {
    return `npx efa install --profile ${id} --target ${target}`;
  }

  return `npx efa install --profile minimal --target ${target} --with ${id}`;
}

function planCommandFor(componentId, target) {
  return `npx efa plan --profile minimal --target ${target} --with ${componentId}`;
}
...
function buildConsultation(options) {
  ...
  return {
    ...
    nextSteps: matches.length > 0
      ...
      : [
        'Run `npx efa catalog components` to browse all components.',
...
function formatText(payload) {
  const lines = [
    `EFA consult (${payload.generatedAt})`,
    `Query: ${payload.query}`,
  ...
  if (payload.matches.length === 0) {
    ...
    lines.push('Try: npx efa catalog components');
  } else {
