...
function detectTargetMode(rootDir) {
  ...
  if (packageJson?.name === 'everything-for-ai') {
    return 'repo';
  }
...
const EFA_PLUGIN_KEY_PATTERNS = [
  /^efa@/i,
  /^everything-for-ai@/i,
];

const EFA_LEGACY_PLUGIN_DIRS = [
  'efa',
  'efa@efa',
  'everything-for-ai',
  'everything-for-ai@everything-for-ai',
];

const EFA_CACHE_MARKETPLACES = ['everything-for-ai', 'efa'];
const EFA_CACHE_PLUGIN_NAMES = ['efa', 'everything-for-ai'];
...
function findPluginInstallFromManifest(installedPluginsPaths) {
  for (const installedPath of installedPluginsPaths) {
    ...
    for (const [key, value] of Object.entries(manifest.plugins)) {
      if (!EFA_PLUGIN_KEY_PATTERNS.some(pattern => pattern.test(key))) {
        continue;
      }
...
function findPluginInstallFlatLayout(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const pluginDir of EFA_LEGACY_PLUGIN_DIRS) {
...
function findPluginInstallMarketplaceCache(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const marketplace of EFA_CACHE_MARKETPLACES) {
      for (const pluginName of EFA_CACHE_PLUGIN_NAMES) {
  ...
  return [
    {
      ...
      points: 4,
      scopes: ['repo'],
      path: '~/.claude/plugins/efa/ (legacy everything-for-ai paths also supported)',
      description: 'Everything For Ai is installed for the active user or project',
      pass: Boolean(pluginInstall),
      fix: 'Install the EFA plugin for this user or project before auditing project-specific harness quality.',
    },
    {
      ...
      fix: 'Add project-local .claude hooks, commands, skills, or settings that tailor EFA to this repo.',
    },
    {
...
Audits the current working directory by default and auto-detects EFA repo mode vs consumer-project mode.
`);
