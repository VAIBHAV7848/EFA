...
# Sync Everything For Ai (EFA) assets into a local Codex CLI setup.
# - Backs up ~/.codex config and AGENTS.md
# - Merges EFA AGENTS.md into existing AGENTS.md (marker-based, preserves user content)
...
# - Merges EFA MCP servers into config.toml (add-only via Node TOML parser)
...
BACKUP_DIR="$CODEX_HOME/backups/efa-$STAMP"

log() { printf '[efa-sync] %s\n' "$*"; }
...
generate_prompt_file() {
  ...
  local cmd_name="$3"
  {
    printf '# EFA Command Prompt: /%s\n\n' "$cmd_name"
    printf 'Source: %s\n\n' "$src"
    printf 'Use this prompt to run the EFA `%s` workflow.\n\n' "$cmd_name"
    awk '
      NR == 1 && $0 == "---" { fm = 1; next }
      fm == 1 && $0 == "---" { fm = 0; next }
      fm == 1 { next }
...
require_path "$REPO_ROOT/AGENTS.md" "EFA AGENTS.md"
require_path "$AGENTS_CODEX_SUPP_SRC" "EFA Codex AGENTS supplement"
require_path "$CODEX_AGENTS_SRC" "EFA Codex agent roles"
require_path "$PROMPTS_SRC" "EFA commands directory"
require_path "$BASELINE_MERGE_SCRIPT" "EFA Codex baseline merge script"
require_path "$HOOKS_INSTALLER" "EFA global git hooks installer"
require_path "$SANITY_CHECKER" "EFA global sanity checker"
require_path "$CURSOR_RULES_DIR" "EFA Cursor rules directory"
require_path "$CONFIG_FILE" "Codex config.toml"
require_path "$MCP_MERGE_SCRIPT" "EFA MCP merge script"
...
EFA_BEGIN_MARKER="<!-- BEGIN EFA -->"
EFA_END_MARKER="<!-- END EFA -->"

compose_efa_block() {
  printf '%s\n' "$EFA_BEGIN_MARKER"
  cat "$AGENTS_ROOT_SRC"
  printf '\n\n---\n\n'
  printf '# Codex Supplement (From EFA .codex/AGENTS.md)\n\n'
  cat "$AGENTS_CODEX_SUPP_SRC"
  printf '\n%s\n' "$EFA_END_MARKER"
}

log "Merging EFA AGENTS into $AGENTS_FILE (preserving user content)"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] merge EFA block into %s from %s + %s\n' "$AGENTS_FILE" "$AGENTS_ROOT_SRC" "$AGENTS_CODEX_SUPP_SRC"
else
  replace_efa_section() {
    # Replace the EFA block between markers in $AGENTS_FILE with fresh content.
    # Uses awk to correctly handle all positions including line 1.
    local tmp
    tmp="$(mktemp)"
    local efa_tmp
    efa_tmp="$(mktemp)"
    compose_efa_block > "$efa_tmp"
    awk -v begin="$EFA_BEGIN_MARKER" -v end="$EFA_END_MARKER" -v efa="$efa_tmp" '
      { gsub(/\r$/, "") }
      $0 == begin { skip = 1; while ((getline line < efa) > 0) print line; close(efa); next }
      $0 == end   { skip = 0; next }
      !skip        { print }
    ' "$AGENTS_FILE" > "$tmp"
  ...
  if [[ ! -f "$AGENTS_FILE" ]]; then
    ...
    compose_efa_block > "$AGENTS_FILE"
  elif awk -v b="$EFA_BEGIN_MARKER" -v e="$EFA_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++; if (!fb) fb = NR }
        $0 == e { ec++; if (!fe) fe = NR }
        END { exit !(bc == 1 && ec == 1 && fb < fe) }
      ' "$AGENTS_FILE"; then
    # Exactly one BEGIN/END pair in correct order — replace only the EFA section
    replace_efa_section
  elif awk -v b="$EFA_BEGIN_MARKER" -v e="$EFA_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++ } $0 == e { ec++ }
    ...
    log "WARNING: EFA markers found but not a clean pair — stripping markers and re-appending"
    _fix_tmp="$(mktemp)"
    awk -v b="$EFA_BEGIN_MARKER" -v e="$EFA_END_MARKER" '
      { gsub(/\r$/, "") }
      $0 == b { skip = 1; next }
      $0 == e { skip = 0; next }
      !skip   { print }
    ...
    # Existing file without markers — append EFA block, preserving existing content.
    # Legacy EFA-only files will have duplicate content after this first run, but
    ...
    log "No EFA markers found — appending managed block (backup saved)"
    {
      printf '\n\n'
      compose_efa_block
    } >> "$AGENTS_FILE"
...
log "Merging EFA Codex baseline into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
# ~/.agents/skills/ (installed by EFA installer / npx skills).
# Copying into ~/.codex/skills/ was unnecessary.

log "Generating prompt files from EFA commands"
run_or_echo mkdir -p "$PROMPTS_DEST"
manifest="$PROMPTS_DEST/efa-prompts-manifest.txt"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] > %s\n' "$manifest"
else
  : > "$manifest"
...
extension_manifest="$PROMPTS_DEST/efa-extension-prompts-manifest.txt"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] > %s\n' "$extension_manifest"
else
  : > "$extension_manifest"
...
write_extension_prompt "efa-tool-run-tests.md" <<EOF
# EFA Tool Prompt: run-tests
...
write_extension_prompt "efa-tool-check-coverage.md" <<EOF
# EFA Tool Prompt: check-coverage
...
write_extension_prompt "efa-tool-security-audit.md" <<EOF
# EFA Tool Prompt: security-audit
...
write_extension_prompt "efa-rules-pack-common.md" <<EOF
# EFA Rule Pack: common (optional)

Apply EFA common engineering rules for this session. Use these files as the source of truth:
...
write_extension_prompt "efa-rules-pack-typescript.md" <<EOF
# EFA Rule Pack: typescript (optional)

Apply EFA common rules plus TypeScript-specific rules for this session.

## Common
Use \`$PROMPTS_DEST/efa-rules-pack-common.md\`.
...
write_extension_prompt "efa-rules-pack-python.md" <<EOF
# EFA Rule Pack: python (optional)

Apply EFA common rules plus Python-specific rules for this session.
...
write_extension_prompt "efa-rules-pack-golang.md" <<EOF
# EFA Rule Pack: golang (optional)

Apply EFA common rules plus Go-specific rules for this session.
...
write_extension_prompt "efa-rules-pack-swift.md" <<EOF
# EFA Rule Pack: swift (optional)

Apply EFA common rules plus Swift-specific rules for this session.
...
log "Merging EFA MCP servers into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  EFA_GLOBAL_HOOKS_DIR="${EFA_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}" \
