#!/usr/bin/env node
// EFA community Discord bot — dependency-free (Node 22+ native WebSocket).
// Slash commands: /efa /help /skill /docs /release
//
// Env: DISCORD_BOT_TOKEN (required), DISCORD_APP_ID (required),
//      EFA_REPO (path to local clone, default ~/GitHub/EFA/everything-for-ai),
//      DISCORD_INVITE (optional, shown in /efa)
//
...
const REPO = process.env.EFA_REPO || join(homedir(), 'GitHub/EFA/everything-for-ai');
const REPO_URL = 'https://github.com/VAIBHAV7848/EFA';
...
const HELP = [
  '**EFA bot commands**',
  '- `/efa` — what EFA is + all the links',
  '- `/skill name:<query>` — look up an EFA skill',
  '- `/docs query:<terms>` — search the EFA docs',
  '- `/release` — latest EFA release',
  '- `/help` — this message',
].join('\n');

const handlers = {
  efa: () => [
    '**Everything For Ai (EFA)** — the agent harness performance system.',
    ...
    '',
    `- repo: ${REPO_URL}`,
    '- site: https://efa.tools',
    `- install: \`/plugin marketplace add VAIBHAV7848/EFA\` then \`/plugin install efa\``,
  ...
  release: async () => {
    const res = await fetch('https://api.github.com/repos/VAIBHAV7848/EFA/releases/latest', {
      headers: { 'User-Agent': 'efa-discord-bot' },
    });
  ...
  ws.onmessage = ev => {
    ...
    switch (msg.op) {
      case 10: { // HELLO
        ...
        send({ op: 2, d: { token: TOKEN, intents: 1, properties: { os: 'darwin', browser: 'efa-bot', device: 'efa-bot' } } });
        break;
      }
