...
/**
 * Resolve session retention days from the EFA_SESSION_RETENTION_DAYS env var.
 *
...
function getSessionRetentionDays() {
  const raw = process.env.EFA_SESSION_RETENTION_DAYS;
...
function isSessionStartContextDisabled() {
  const raw = String(process.env.EFA_SESSION_START_CONTEXT || '').trim().toLowerCase();
  return ['0', 'false', 'off', 'none', 'disabled'].includes(raw);
}

function getSessionStartMaxContextChars() {
  const raw = process.env.EFA_SESSION_START_MAX_CHARS;
...
function limitSessionStartContext(additionalContext, maxChars = getSessionStartMaxContextChars()) {
  ...
  const marker = '\n\n[SessionStart truncated context. Set EFA_SESSION_START_MAX_CHARS to raise the cap or EFA_SESSION_START_CONTEXT=off to disable injected context.]';
...
async function main() {
  ...
  if (retentionDays === null) {
    log('[SessionStart] Pruning disabled via EFA_SESSION_RETENTION_DAYS');
  } else {
  ...
  if (explicitContextDisabled) {
    log('[SessionStart] Additional context injection disabled by EFA_SESSION_START_CONTEXT');
  } else if (maxContextChars === 0) {
    log('[SessionStart] Additional context injection disabled by EFA_SESSION_START_MAX_CHARS=0');
  }
      ...
      if (recentSessions.length > 0) {
        ...
        if (result) {
          ...
          if (content && !content.includes('[Session context goes here]')) {
            ...
            // https://github.com/VAIBHAV7848/EFA/issues/1534
            const guarded = [
