#!/usr/bin/env node
/**
 * EFA Context Monitor — PostToolUse hook
 *
 * Reads bridge file from efa-metrics-bridge.js and injects agent-facing
...
function costWarningsEnabled(env = process.env) {
  return isEnabledEnv(env.EFA_CONTEXT_MONITOR_COST_WARNINGS, true);
}
...
function getWarnPath(sessionId) {
  return path.join(os.tmpdir(), `efa-ctx-warn-${sessionId}.json`);
}
 ...
 * and `writeCostWarningIfChanged` in `scripts/hooks/efa-metrics-bridge.js`.
 *
 * @param {string} sessionId
 * @param {object} state
 */
