#!/usr/bin/env node
/**
 * Executes a hook script only when enabled by EFA hook profile flags.
 *
 * Usage:
...
async function main() {
  ...
  // the truncated flag (run() context / EFA_HOOK_INPUT_TRUNCATED), so
  ...
  const result = spawnSync(process.execPath, [scriptPath], {
    ...
    env: {
      ...
      EFA_HOOK_ID: hookId,
      EFA_HOOK_INPUT_TRUNCATED: truncated ? '1' : '0',
      EFA_HOOK_INPUT_MAX_BYTES: String(MAX_STDIN)
    },
    cwd: process.cwd(),
    timeout: 30000
  });
