 ...
 *
 * Bootstrap loader for the EFA SessionStart hook.
 *
 ...
 *   2. Resolves the EFA plugin root directory (via CLAUDE_PLUGIN_ROOT env var
...
const CURRENT_PLUGIN_SLUG = 'efa';
const LEGACY_PLUGIN_SLUG = 'everything-for-ai';
...
/**
 * Returns true when `candidate` looks like a valid EFA plugin root, i.e. the
...
/**
 * Resolves the EFA plugin root using the following priority order:
 ...
 *   4. Versioned cache directories under ~/.claude/plugins/cache/{efa,everything-for-ai}/
...
function resolvePluginRoot() {
  ...
  // Walk versioned cache: ~/.claude/plugins/cache/{efa,everything-for-ai}/<org>/<version>/
  try {
...
process.stderr.write(
  '[SessionStart] WARNING: could not resolve EFA plugin root; skipping session-start hook\n'
);
process.stdout.write(raw);
