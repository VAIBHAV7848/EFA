#!/usr/bin/env node
/**
 * EFA Metrics Bridge — PostToolUse hook
 *
 * Maintains a running session aggregate in /tmp/efa-metrics-{session}.json.
 * This bridge file is read by efa-statusline.js and efa-context-monitor.js,
...
const WARNING_CACHE_PREFIX = 'efa-metrics-cost-warnings-';
  ...
  try {
    ...
    if (malformed > 0) {
      writeCostWarningIfChanged(
        ...
        `[efa-metrics-bridge] skipped ${malformed} malformed line(s) in ${costsPath}\n`
      );
    }
    ...
    if (err && err.code !== 'ENOENT') {
      writeCostWarningIfChanged(
        ...
        `[efa-metrics-bridge] failing open after ${err.name || 'error'} reading ${costsPath}: ${err.message || String(err)}\n`
      );
    }
...
function run(rawInput) {
  try {
    ...
    const sessionId = sanitizeSessionId(input.session_id) || sanitizeSessionId(process.env.EFA_SESSION_ID) || sanitizeSessionId(process.env.CLAUDE_SESSION_ID);
