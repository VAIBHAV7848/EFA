...
Pull the latest EFA repo changes and reinstall the current context's managed targets
...
function deriveRepoRootFromState(state) {
  ...
  throw new Error('Unable to infer EFA repo root from install-state operations');
}
...
function validateRepoRoot(repoRoot) {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    throw new Error(`Invalid EFA repo root: missing package.json at ${packageJsonPath}`);
  }

  if (!fs.existsSync(installApplyPath)) {
    throw new Error(`Invalid EFA repo root: missing install script at ${installApplyPath}`);
  }
...
function runAutoUpdate(options = {}, dependencies = {}) {
  ...
  if (!requestedRepoRoot) {
    ...
    if (uniqueRepoRoots.length > 1) {
      throw new Error(`Multiple EFA repo roots detected: ${uniqueRepoRoots.join(', ')}`);
    }
  }
...
function printHuman(result) {
  if (result.results.length === 0) {
    console.log('No EFA install-state files found for the current home/project context.');
    return;
  }
