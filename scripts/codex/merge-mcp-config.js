...
/**
 * Merge EFA-recommended MCP servers into a Codex config.toml.
 *
 ...
 *   - Log warnings when an existing server's config differs from the EFA recommendation.
 *   - With --update-mcp, also replace existing EFA-managed servers.
 *
...
try {
  ...
  console.error('[efa-mcp] Missing dependency: @iarna/toml');
  console.error('[efa-mcp] Run: npm install   (from the EFA repo root)');
  process.exit(1);
}
...
// EFA <= 2.0.0 emitted [mcp_servers.exa] with a `url` key. Codex rejects
// `url` for stdio servers, which makes the *entire* config.toml fail to
// load (#2224). Repair exactly that EFA-emitted form on every merge so
...
// Legacy section names that should be treated as an existing EFA server.
...
function log(msg) {
  console.log(`[efa-mcp] ${msg}`);
}

function warn(msg) {
  console.warn(`[efa-mcp] WARNING: ${msg}`);
}
...
function main() {
  ...
  const disabledServers = new Set(parseDisabledMcpServers(process.env.EFA_DISABLED_MCPS));
  ...
  if (!fs.existsSync(configPath)) {
    console.error(`[efa-mcp] Config file not found: ${configPath}`);
    process.exit(1);
  }

  log(`Package manager: ${PM_NAME} (exec: ${PM_EXEC})`);
  if (disabledServers.size > 0) {
    log(`Disabled via EFA_DISABLED_MCPS: ${[...disabledServers].join(', ')}`);
  }
  ...
  try {
    ...
    console.error(`[efa-mcp] Failed to parse ${configPath}: ${err.message}`);
    process.exit(1);
  }
  ...
  // Repair schema-invalid entries emitted by earlier EFA versions (#2224).
  for (const [name, invalidUrl] of Object.entries(RETIRED_INVALID_URL_SERVERS)) {
    ...
    if (isBrokenEccForm) {
      toRemoveLog.push(`mcp_servers.${name} (invalid url entry from earlier EFA versions)`);
      raw = removeServerFromText(raw, name, existing);
      log(`  [repair] mcp_servers.${name} — url is not valid for Codex stdio servers, removing`);
    }
  }

  for (const [name, spec] of Object.entries(EFA_SERVERS)) {
    const entry = existing[name];
    ...
    // (Codex itself rejects url-form stdio servers; EFA only ever emits
    ...
    if (finalEntry) {
      if (updateMcp) {
        ...
        if (legacyName && !hasCanonical) {
          ...
          warn(`mcp_servers.${name} differs from EFA recommendation (run with --update-mcp to refresh)`);
        } else {
  ...
  if (toAppend.length === 0 && !hasRemovals) {
    log('All EFA MCP servers already present. Nothing to do.');
    return;
  }
