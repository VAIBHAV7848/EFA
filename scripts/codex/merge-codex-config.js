...
try {
  ...
  console.error('[efa-codex] Missing dependency: @iarna/toml');
  console.error('[efa-codex] Run: npm install   (from the EFA repo root)');
  process.exit(1);
}
...
function warn(message) {
  console.warn(`[efa-codex] WARNING: ${message}`);
}
...
function main() {
  ...
  if (!fs.existsSync(referencePath)) {
    console.error(`[efa-codex] Reference config not found: ${referencePath}`);
    process.exit(1);
  }

  if (!fs.existsSync(configPath)) {
    console.error(`[efa-codex] Config file not found: ${configPath}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[efa-codex] Failed to parse TOML: ${error.message}`);
    process.exit(1);
  }
