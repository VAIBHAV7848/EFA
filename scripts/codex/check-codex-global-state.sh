...
# EFA Codex global regression sanity check.
# Validates that global ~/.codex state matches expected EFA integration.
...
HOOKS_DIR_EXPECT="${EFA_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}"
...
printf 'EFA GLOBAL SANITY CHECK\n'
printf 'Repo: %s\n' "$REPO_ROOT"
printf 'Codex home: %s\n\n' "$CODEX_HOME"
...
if [[ -f "$AGENTS_FILE" ]]; then
  if search_file '^# Everything For Ai \(EFA\)' "$AGENTS_FILE"; then
    ok "AGENTS contains EFA root instructions"
  else
    fail "AGENTS missing EFA root instructions"
  fi

  if search_file '^# Codex Supplement \(From EFA \.codex/AGENTS\.md\)' "$AGENTS_FILE"; then
    ok "AGENTS contains EFA Codex supplement"
  else
    fail "AGENTS missing EFA Codex supplement"
  fi
fi
...
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  # EFA <= 2.0.0 emitted a url-only exa entry that Codex's stdio-only
  ...
  if search_file '^\[mcp_servers\.exa\]' "$CONFIG_FILE"; then
    ...
    if printf '%s\n' "$exa_block" | grep -Eq '^[[:space:]]*url[[:space:]]*=' \
      ...
      fail "MCP section [mcp_servers.exa] uses a url key, which Codex rejects for stdio servers — re-run efa-sync-codex to repair (#2224)"
    else
...
if [[ -d "$SKILLS_DIR" ]]; then
  ...
  if [[ "$missing_skills" -eq 0 ]]; then
    ok "All 16 EFA skills are present in $SKILLS_DIR"
  else
    warn "$missing_skills EFA skills missing from $SKILLS_DIR (install via EFA installer or npx skills)"
  fi
else
  warn "Skills directory missing ($SKILLS_DIR) — install via EFA installer or npx skills"
fi

if [[ -f "$PROMPTS_DIR/efa-prompts-manifest.txt" ]]; then
  ok "Command prompts manifest exists"
else
  fail "Command prompts manifest missing"
fi

if [[ -f "$PROMPTS_DIR/efa-extension-prompts-manifest.txt" ]]; then
  ok "Extension prompts manifest exists"
else
  fail "Extension prompts manifest missing"
fi

command_prompts_count="$(find "$PROMPTS_DIR" -maxdepth 1 -type f -name 'efa-*.md' 2>/dev/null | wc -l | tr -d ' ')"
if [[ "$command_prompts_count" -ge 43 ]]; then
  ok "EFA prompts count is $command_prompts_count (expected >= 43)"
else
  fail "EFA prompts count is $command_prompts_count (expected >= 43)"
fi
...
if command -v efa-sync-codex >/dev/null 2>&1; then
  ok "efa-sync-codex command is in PATH"
else
  warn "efa-sync-codex is not in PATH"
fi

if command -v efa-install-git-hooks >/dev/null 2>&1; then
  ok "efa-install-git-hooks command is in PATH"
else
  warn "efa-install-git-hooks is not in PATH"
fi

if command -v efa-check-codex >/dev/null 2>&1; then
  ok "efa-check-codex command is in PATH"
else
  warn "efa-check-codex is not in PATH (this is expected before alias setup)"
fi

printf '\nSummary: checks=%d, warnings=%d, failures=%d\n' "$checks" "$warnings" "$failures"
if [[ "$failures" -eq 0 ]]; then
  printf 'EFA GLOBAL SANITY: PASS\n'
else
  printf 'EFA GLOBAL SANITY: FAIL\n'
  exit 1
fi
