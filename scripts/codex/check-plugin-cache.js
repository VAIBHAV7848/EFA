...
function usage() {
  console.log([
    ...
    '  --marketplace <name> Marketplace cache name (default: efa)',
    '  --plugin <name>      Plugin cache name (default: efa)',
...
function checkCache(options) {
  ...
  if (!fs.existsSync(manifestPath)) {
    ...
    if (versions.length > 0) {
      ...
      if (options.marketplace === 'efa' && options.plugin === 'efa') {
        log('Run: codex plugin marketplace add VAIBHAV7848/EFA');
      } else {
  ...
  if (failures > 0) {
    ...
    const syncScript = path.join(REPO_ROOT, 'scripts', 'sync-efa-to-codex.sh');
    if (fs.existsSync(syncScript)) {
      log('Use the supported sync path until the cache contains the referenced files:');
      log('npm install && bash scripts/sync-efa-to-codex.sh');
    } else {
      log('Use the supported manual sync workflow from your EFA installation.');
    }
    return 1;
  }
...
function main() {
  ...
  try {
    ...
    console.error(`[efa-codex] ${error.message}`);
    usage();
    process.exit(1);
  }
