...
- Discord community launch: server + GitHub PR/issue/release feed, `release-announce.yml` workflow (announce + pin + Discussions cross-post), and a dependency-free community bot (`scripts/discord/efa-bot.mjs`) with `/efa`, `/help`, `/skill`, `/docs`, `/release`.
- `orch-*` orchestrator skill family and dynamic workflow team orchestration.
- `kubernetes-patterns` skill, worktree-lifecycle service, MCP inventory (`efa.mcp.v1`), codex-worktree and opencode session adapters.
...
- Adds the public EFA 2.0 release-candidate surface for the Hermes operator story.
- Documents EFA as the reusable cross-harness substrate across Claude Code, Codex, Cursor, OpenCode, and Gemini.
...
- Added `docs/architecture/cross-harness.md` and regression coverage for the EFA/Hermes boundary.
- Kept `efa2/` versioning independent for now; it remains an alpha control-plane scaffold unless release engineering decides otherwise.

### Notes

- This is a release candidate, not a GA claim for the full EFA 2.0 control-plane roadmap.
...
- EFA 2.0 alpha control-plane binary now builds locally from `efa2/` and exposes the first usable CLI/TUI surface.
...
- `cargo build --manifest-path efa2/Cargo.toml` passes on the repository baseline.
- `efa-tui` currently exposes `dashboard`, `start`, `sessions`, `status`, `stop`, `resume`, and `daemon`.
...
- This release is a repo-surface correction and ecosystem sync, not a claim that the full EFA 2.0 roadmap is complete.
...
- `efa-devfleet` — Multi-agent orchestration (#505)
- `blueprint` — Multi-session construction planning
- `everything-for-ai` — Self-referential EFA skill (#335)
...
- Added `EFA_HOOK_PROFILE` and `EFA_DISABLED_HOOKS` controls.
