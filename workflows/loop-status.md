...
```bash
npx --package efa-universal efa loop-status --json
```
...
- `efa loop-status --json` emits machine-readable status for recent local
  Claude transcripts.
- `efa loop-status --home <dir>` scans a different home directory when
  inspecting another local profile or mounted workspace.
- `efa loop-status --transcript <session.jsonl>` inspects one transcript
  directly.
- `efa loop-status --bash-timeout-seconds 1800` adjusts the stale Bash
  threshold.
- `efa loop-status --exit-code` exits `2` when stale loop or tool signals are
...
- `efa loop-status --watch` refreshes status until interrupted.
- `efa loop-status --watch --watch-count 3 --exit-code` refreshes a bounded
  number of times, then exits with the highest status seen.
- `efa loop-status --watch --watch-count 3` emits a bounded watch stream for
  scripts and handoffs.
- `efa loop-status --watch --write-dir ~/.claude/loops` maintains
...
timeout Everything For Ai runtime tool calls.
