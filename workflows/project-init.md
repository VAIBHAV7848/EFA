---
description: Detect a project's stack and produce a dry-run EFA onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable EFA onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
/project-init --config efa-install.json
```
...
3. Use EFA's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
- EFA config: `efa-install.json`
- optional stack map: `config/project-stack-mappings.json` in the EFA repo

When the EFA checkout is available, use `config/project-stack-mappings.json` as the stack-to-rules/skills reference. If the file is unavailable, fall back to the installed EFA manifests and explicit user choices.
...
3. Resolve the smallest useful EFA plan:
   - project has an `efa-install.json`: `node scripts/install-plan.js --config efa-install.json --json`
...
- `/efa-guide` for interactive feature discovery before installing
