...
const opencodeHooksPluginPath = path.join(repoRoot, '.opencode', 'plugins', 'efa-hooks.ts');
...
test('agent.yaml uses canonical EFA identity', () => {
  const agentYamlSource = fs.readFileSync(agentYamlPath, 'utf8');
  assert.ok(/^name:\s*efa$/m.test(agentYamlSource), 'Expected agent.yaml to use the efa name');
});
...
test('.opencode/plugins/efa-hooks.ts active plugin banner matches package.json', () => {
  const source = fs.readFileSync(opencodeHooksPluginPath, 'utf8');
  const match = source.match(new RegExp(`## Active Plugin: EFA v(${semverPattern})`));
  assert.ok(match, 'Expected .opencode/plugins/efa-hooks.ts to declare an active plugin banner');
  assert.strictEqual(match[1], expectedVersion);
});
...
test('claude plugin.json uses short plugin slug', () => {
  assert.strictEqual(claudePlugin.name, 'efa');
});
...
test('claude plugin.json disables bundled MCP servers for provider tool-name compatibility', () => {
  const legacyPluginName = 'everything-for-ai';
...
test('claude marketplace.json has plugins array with the published plugin entry', () => {
  ...
  assert.strictEqual(claudeMarketplace.name, 'efa');
  assert.strictEqual(claudeMarketplace.plugins[0].name, 'efa');
});
...
test('codex plugin.json uses short plugin slug', () => {
  assert.strictEqual(codexPlugin.name, 'efa');
});
...
test('codex plugin.json uses canonical EFA repo and display name', () => {
  assert.strictEqual(codexPlugin.repository, 'https://github.com/VAIBHAV7848/EFA');
  assert.strictEqual(codexPlugin.interface.displayName, 'EFA');
});
...
test('marketplace.json uses short marketplace slug', () => {
  assert.strictEqual(marketplace.name, 'efa');
});
...
test('marketplace.json plugin entry uses short plugin slug', () => {
  assert.strictEqual(marketplace.plugins[0].name, 'efa');
});
...
// ── plugins/efa marketplace plugin folder ─────────────────────────────────────
...
console.log('\n=== plugins/efa Codex marketplace plugin folder ===\n');

const marketplacePluginManifestPath = path.join(repoRoot, 'plugins', 'efa', '.codex-plugin', 'plugin.json');
const marketplacePluginManifest = loadJsonObject(marketplacePluginManifestPath, 'plugins/efa/.codex-plugin/plugin.json');
const rootCodexManifest = loadJsonObject(path.join(repoRoot, '.codex-plugin', 'plugin.json'), '.codex-plugin/plugin.json');

test('plugins/efa manifest name matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.name, rootCodexManifest.name);
});

test('plugins/efa manifest version matches package.json', () => {
  assert.strictEqual(marketplacePluginManifest.version, expectedVersion);
});

test('plugins/efa manifest version matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.version, rootCodexManifest.version);
});

test('plugins/efa manifest reuses root skills and MCP config without vendoring', () => {
  ...
  assert.ok(!fs.existsSync(path.join(pluginDir, 'skills')), 'plugins/efa must not vendor a second skills/ copy (see #2097 review)');
  assert.ok(!fs.existsSync(path.join(pluginDir, '.mcp.json')), 'plugins/efa must not vendor a second .mcp.json (see #2097 review)');
});

test('plugins/efa manifest interface assets resolve to root assets', () => {
  const pluginDir = path.dirname(path.dirname(marketplacePluginManifestPath));
...
test('plugins/efa README documents the upstream Codex fragility', () => {
  const readmePath = path.join(repoRoot, 'plugins', 'efa', 'README.md');
  assert.ok(fs.existsSync(readmePath), 'Expected plugins/efa/README.md');
  const source = fs.readFileSync(readmePath, 'utf8');
  assert.ok(source.includes('openai/codex'), 'plugins/efa README must link the upstream Codex discovery issue');
  assert.ok(source.includes('check-plugin-cache.js'), 'plugins/efa README must point at the cache health check');
  assert.ok(source.includes('sync-efa-to-codex.sh'), 'plugins/efa README must point at the supported manual sync flow');
});
...
test('user-facing docs do not use overlong legacy marketplace install commands', () => {
  const markdownFiles = [
    ...
    path.join(repoRoot, 'skills', 'configure-efa', 'SKILL.md'),
  ...
  for (const filePath of markdownFiles) {
    ...
    if (/\/plugin\s+(install|list)\s+everything-for-ai(?:@everything-for-ai)?\b/.test(source)) {
...
test('user-facing docs do not use the legacy non-URL marketplace add form', () => {
  ...
  for (const filePath of markdownFiles) {
    const source = fs.readFileSync(filePath, 'utf8');
    if (source.includes('/plugin marketplace add VAIBHAV7848/EFA')) {
      offenders.push(path.relative(repoRoot, filePath));
    }
  }
...
test('.codex-plugin README uses current marketplace add flow', () => {
  ...
  assert.ok(readme.includes('codex plugin marketplace add VAIBHAV7848/EFA'), 'Expected .codex-plugin README to document the canonical EFA repo marketplace source');
