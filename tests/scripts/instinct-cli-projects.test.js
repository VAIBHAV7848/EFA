...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'efa-instinct-cli-projects-'));
}
...
test('status migrates legacy no-remote linked worktree project dirs to main worktree id', () => {
  ...
  try {
    ...
    runGit(mainWorktree, ['config', 'user.email', 'efa@example.test']);
    runGit(mainWorktree, ['config', 'user.name', 'EFA Test']);
