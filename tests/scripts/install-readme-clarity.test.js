...
function runTests() {
  ...
  if (test('README documents reset and uninstall flow', () => {
    assert.ok(
      readme.includes('### Reset / Uninstall EFA'),
    ...
    assert.ok(
      readme.includes('node scripts/efa.js list-installed'),
      'README should document install-state inspection before reinstalling'
    );
    assert.ok(
      readme.includes('node scripts/efa.js doctor'),
      'README should document doctor before reinstalling'
    );
    assert.ok(
      readme.includes('EFA only removes files recorded in its install-state.'),
  ...
  if (test('README documents low-context no-hooks install path', () => {
    ...
    assert.ok(
      readme.includes('npx efa-install --profile minimal --target claude'),
  ...
  if (test('README documents consult-based component discovery', () => {
    ...
    assert.ok(
      readme.includes('npx efa consult "security reviews" --target claude'),
  ...
  if (test('README documents Cursor agent namespace and loading caveat', () => {
    assert.ok(
      readme.includes('`.cursor/agents/efa-*.md`'),
    ...
    assert.ok(
      readme.includes('EFA does not install root `AGENTS.md` into `.cursor/`.'),
  ...
  if (test('README explains plugin-path cleanup and rules scoping', () => {
    ...
    assert.ok(
      readme.includes('~/.claude/rules/efa/'),
      'README should steer plugin-path rules into an EFA-owned namespace'
    );
  })) passed++; else failed++;

  if (test('rules README mirrors EFA namespaced install path', () => {
    assert.ok(
      rulesReadme.includes('mkdir -p ~/.claude/rules/efa'),
      'rules README should create the EFA-owned user-level rules namespace'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/common ~/.claude/rules/efa/'),
      'rules README should copy common rules under ~/.claude/rules/efa/'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/typescript ~/.claude/rules/efa/'),
      'rules README should copy language rules under ~/.claude/rules/efa/'
    );
    assert.ok(
      rulesReadme.includes('mkdir -p .claude/rules/efa'),
      'rules README should document the project-local EFA namespace'
    );
    assert.ok(
