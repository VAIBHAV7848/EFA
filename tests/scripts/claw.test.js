...
// Test helper — matches EFA's custom test pattern
...
function runTests() {
  ...
  if (test('loadEFAContext() returns "" when no skills specified', () => {
    const result = loadEFAContext('');
    assert.strictEqual(result, '');
  })) passed++; else failed++;

  if (test('loadEFAContext() skips missing skill directories gracefully', () => {
    const result = loadEFAContext('nonexistent-skill-xyz');
    assert.strictEqual(result, '');
  })) passed++; else failed++;

  if (test('loadEFAContext() concatenates multiple skill files', () => {
    // Use real skills from the EFA repo if they exist
    ...
    const result = loadEFAContext(twoSkills);
  ...
  if (test('module exports all required functions', () => {
    ...
    const required = [
      ...
      'appendTurn', 'loadEFAContext', 'askClaude', 'main'
    ];
