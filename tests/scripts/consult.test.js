...
function runTests() {
  ...
  if (test('shows help with an explicit help flag', () => {
    ...
    assert.match(result.stdout, /Consult EFA install components/);
  ...
  if (test('recommends security components and profile for a natural language query', () => {
    ...
    assert.strictEqual(payload.schemaVersion, 'efa.consult.v1');
    ...
    assert.strictEqual(
      ...
      'npx efa install --profile minimal --target claude --with capability:security'
    );
  ...
  if (test('prints text recommendations with install and plan commands', () => {
    ...
    assert.strictEqual(result.status, 0, result.stderr);
    assert.match(result.stdout, /EFA consult/);
    assert.match(result.stdout, /capability:security/);
    assert.match(result.stdout, /npx efa install --profile minimal --target claude --with capability:security/);
    assert.match(result.stdout, /npx efa plan --profile minimal --target claude --with capability:security/);
  })) passed++; else failed++;
  ...
  if (test('works from outside the EFA repository', () => {
    const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-consult-project-'));
    try {
