...
function runTests() {
  ...
  if (test('shows help with no arguments', () => {
    ...
    assert.ok(result.stdout.includes('Inspect EFA selective-install manifests'));
  })) passed++; else failed++;
  ...
  if (test('loads planning intent from efa-install.json', () => {
    const configDir = path.join(__dirname, '..', 'fixtures', 'tmp-install-plan-config');
    const configPath = path.join(configDir, 'efa-install.json');
  ...
  if (test('auto-detects planning intent from project efa-install.json', () => {
