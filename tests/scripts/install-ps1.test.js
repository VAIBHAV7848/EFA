...
function runTests() {
  ...
  if (test('publishes efa-install through the Node installer runtime for cross-platform npm usage', () => {
    const packageJson = JSON.parse(fs.readFileSync(PACKAGE_JSON, 'utf8'));
    assert.strictEqual(packageJson.bin['efa-install'], 'scripts/install-apply.js');
  })) passed++; else failed++;
  ...
  if (!powerShellCommand) {
    ...
    assert.strictEqual(result.code, 0, result.stderr);
    assert.ok(
      result.stdout.includes('claude       (default) - Install EFA into ~/.claude/'),
