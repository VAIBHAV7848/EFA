...
function runTests() {
  ...
  if (test('seeded approved release passes every publication approval check', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'efa.release-approval-gate.v1');
    ...
    try {
      seedRepo(rootDir, {
        [`${RELEASE_DIR}/release-url-ledger-2026-05-19.md`]: [
          ...
          '| --- | --- | --- |',
          '| GitHub prerelease | https://github.com/VAIBHAV7848/EFA/releases/tag/v2.0.0-rc.1 | must return the prerelease |',
        ].join('\n'),
      });
  ...
  if (test('announcement drafts fail on unresolved placeholders and private paths', () => {
    ...
    try {
      seedRepo(rootDir, {
        [`${RELEASE_DIR}/x-thread.md`]: 'Ship copy with <video-url> and /Users/VAIBHAV7848/raw-footage.',
      });
