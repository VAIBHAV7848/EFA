...
const SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'efa.js');
...
function main() {
  ...
  const tests = [
    ['shows top-level help', () => {
      ...
      assert.match(result.stdout, /EFA selective-install CLI/);
    ...
    ['delegates lifecycle commands', () => {
      const homeDir = createTempDir('efa-cli-home-');
      const projectRoot = createTempDir('efa-cli-project-');
    ...
    ['delegates session-inspect command', () => {
      ...
      assert.strictEqual(payload.workers[0].branch, 'feat/efa-cli');
    }],
    ['delegates loop-status command', () => {
      const homeDir = createTempDir('efa-cli-home-');
      const transcriptDir = path.join(homeDir, '.claude', 'projects', '-tmp-efa');
    ...
    ['delegates security-ioc-scan command', () => {
      const projectRoot = createTempDir('efa-cli-ioc-scan-');
      fs.writeFileSync(
