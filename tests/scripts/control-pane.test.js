...
async function writeMinimalDatabase(dbPath) {
  ...
  db.run(`
    ...
    INSERT INTO sessions (
      ...
      'session-a', 'Build the control pane', 'codex', 'codex', '["codex"]', '/repo/efa',
...
function waitForCliReady(child) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', chunk => {
      ...
      if (!settled && stdout.includes('EFA Control Pane:') && stdout.includes('Actions:')) {
        settled = true;
        clearTimeout(timer);
...
async function runTests() {
  ...
  if (await test('parses CLI arguments for local-only serving', async () => {
    ...
    assert.strictEqual(parsed.dbPath, '/tmp/efa2.db');
    assert.strictEqual(parsed.stateDbPath, '/tmp/efa-state.db');
  ...
  if (await test('serves HTML and snapshot JSON from a temp EFA2 database', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-server-'));
    ...
    try {
      ...
      try {
        ...
        assert.ok(html.includes('EFA Control Pane'));
  ...
  if (await test('serves health, asset, not-found, invalid body, and read-only action responses', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-routes-'));
    ...
    try {
      ...
      try {
        ...
        try {
          const realAsset = await fetchLocal(`${realAssetApp.url}/assets/efa-icon.svg`);
        ...
        const missingAsset = await fetchLocal(`${app.url}/assets/efa-icon.svg`);
  ...
  if (await test('runAction captures success, failure, and bounded output', async () => {
    ...
    const spawnError = await runAction({
      id: 'spawn-error',
      command: 'definitely-not-efa-control-pane-command',
      args: [],
      cwd: repoRoot,
    });
  ...
  if (await test('CLI starts a read-only local server and shuts down on SIGTERM', async () => {
    const child = spawn(process.execPath, [SCRIPT, '--host', '127.0.0.1', '--port', '0', '--read-only', '--no-open'], {
      ...
      env: {
        ...process.env,
        EFA2_DB_PATH: path.join(os.tmpdir(), 'missing-efa2-cli.db'),
      },
    });
    ...
    try {
      ...
      assert.match(ready.stdout, /EFA Control Pane: http:\/\/127\.0\.0\.1:\d+/);
