...
function run(args = [], options = {}) {
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      EFA_PLUGIN_ROOT: options.eccRoot || '',
      ...(options.env || {}),
    },
...
function runTests() {
  ...
  if (test('normalizes Windows Git Bash POSIX drive roots', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/efa', 'win32'),
      'C:/Users/x/.claude/plugins/efa'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/z/Work/EFA/scripts/hooks/check-console-log.js', 'win32'),
      'Z:/Work/EFA/scripts/hooks/check-console-log.js'
    );
  })) passed++; else failed++;
  ...
  if (test('leaves already-Windows roots unchanged', () => {
    ...
    assert.strictEqual(
      normalizePluginRootForPlatform('D:\\Users\\x\\.claude\\plugins\\efa', 'win32'),
      'D:\\Users\\x\\.claude\\plugins\\efa'
    );
  })) passed++; else failed++;

  if (test('leaves POSIX-looking roots unchanged off Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/efa', 'darwin'),
      '/c/Users/x/.claude/plugins/efa'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/efa', 'linux'),
      '/c/Users/x/.claude/plugins/efa'
    );
  })) passed++; else failed++;
  ...
  if (test('does not mangle UNC or non-drive absolute paths on Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('\\\\server\\share\\efa', 'win32'),
      '\\\\server\\share\\efa'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/workspace/efa', 'win32'),
      '/workspace/efa'
    );
  })) passed++; else failed++;
...
process.stdout.write(JSON.stringify({
  ...
  eccRoot: process.env.EFA_PLUGIN_ROOT,
}));
`);
