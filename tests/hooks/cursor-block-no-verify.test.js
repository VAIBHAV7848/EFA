...
function runWrapper(input, env = {}) {
  ...
  const result = spawnSync('node', [wrapper], {
    input: rawInput,
    encoding: 'utf8',
    env: { ...process.env, EFA_HOOK_PROFILE: 'standard', ...env },
    timeout: 15000,
...
// --- Disable via EFA_DISABLED_HOOKS ---

if (test('respects EFA_DISABLED_HOOKS=pre:bash:block-no-verify', () => {
  const r = runWrapper(
    { command: 'git commit --no-verify -m "msg"' },
    { EFA_DISABLED_HOOKS: 'pre:bash:block-no-verify' }
  );
