...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'efa-observe-subdir-test-'));
}
...
function gitInit(dir) {
  ...
  const remoteResult = spawnSync(
    'git',
    ['remote', 'add', 'origin', 'https://github.com/example/efa-test.git'],
...
function readSingleProjectMetadata(homeDir) {
  const projectsDir = path.join(homeDir, '.local', 'share', 'efa-homunculus', 'projects');
