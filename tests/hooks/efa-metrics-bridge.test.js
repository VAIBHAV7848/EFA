/**
 * Tests for scripts/hooks/efa-metrics-bridge.js
 *
 * Run with: node tests/hooks/efa-metrics-bridge.test.js
 */
...
const { run, hashToolCall, extractFilePaths, readSessionCost } = require('../../scripts/hooks/efa-metrics-bridge');
...
function makeTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'efa-metrics-bridge-test-'));
}

function runTests() {
  console.log('\n=== Testing efa-metrics-bridge.js ===\n');
  ...
  if (
    test('readSessionCost suppresses repeated malformed breadcrumbs across hook subprocesses', () => {
      ...
      try {
        ...
        const bridgePath = path.resolve(__dirname, '../../scripts/hooks/efa-metrics-bridge');
