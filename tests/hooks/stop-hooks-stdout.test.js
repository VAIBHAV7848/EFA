 ...
 *
 * Stop hooks follow the EFA pass-through convention (echo stdin on stdout).
...
const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-stop-stdout-')); // non-git cwd
const dataHome = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-stop-data-'));
...
function hookEnv() {
  const env = {
    ...
    EFA_AGENT_DATA_HOME: dataHome,
    CLAUDE_SESSION_ID: `stop-stdout-test-${process.pid}`
  };
  delete env.EFA_GATEGUARD;
  delete env.EFA_DISABLED_HOOKS;
  return env;
}
