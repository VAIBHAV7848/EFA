...
async function loadPlugin() {
  ...
  const pluginUrl = pathToFileURL(
    path.join(repoRoot, ".opencode", "dist", "plugins", "efa-hooks.js")
  ).href
...
async function withTempProject(files, fn) {
  const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "efa-opencode-plugin-"))
  try {
    ...
    [
      ...
      async () => withTempProject(
        ...
        async (projectDir) => {
          ...
          // Verify EFA_VERSION is not hardcoded
          assert.ok(env.EFA_VERSION !== "1.8.0", "EFA_VERSION should not be hardcoded to 1.8.0")
          assert.ok(env.EFA_VERSION.match(/^\d+\.\d+\.\d+$/), "EFA_VERSION should be a valid semver version")
        }
      ),
    ],
    [
      ...
      async () => withTempProject(["CLAUDE.md"], async (projectDir) => {
        ...
        assert.deepStrictEqual($.calls, [], `Unexpected shell probes: ${$.calls.join(", ")}`)
        assert.ok(
          client.logs.some((entry) => entry.message === "[EFA] Found CLAUDE.md - loading project context"),
