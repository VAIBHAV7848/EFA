...
console.log('\n=== Testing EFA platform value loop docs ===\n');

test('platform value loop doc defines the three-layer EFA 2.0 direction', () => {
  ...
  for (const marker of [
    ...
    'reproducible demo',
    'EFA can be used full-stack as a meta-harness + agent + control pane',
  ]) {
...
test('Ito example preserves non-advisory and gated-access boundaries', () => {
  ...
  for (const marker of [
    ...
    'do not merge EFA Tools billing with Ito billing',
  ]) {
...
test('platform value loop does not overclaim release status or trading ability', () => {
  ...
  const forbidden = [
    ...
    'public EFA skills place trades',
  ];
