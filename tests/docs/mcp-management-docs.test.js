...
test('token optimization guide separates Claude MCP disables from EFA config filters', () => {
  ...
  assert.ok(
    source.includes('`EFA_DISABLED_MCPS` only affects EFA-generated MCP config output'),
    'Token guide should scope EFA_DISABLED_MCPS to config generation'
  );
  assert.ok(
...
test('README MCP guidance avoids settings.json disable instructions', () => {
  ...
  assert.ok(
    source.includes('`EFA_DISABLED_MCPS` is an EFA install/sync filter, not a live Claude Code toggle.'),
    'README should explain EFA_DISABLED_MCPS scope'
  );
  assert.ok(
