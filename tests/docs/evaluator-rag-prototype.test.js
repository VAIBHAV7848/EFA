...
test('fixtures use one scenario id and declare read-only behavior', () => {
  ...
  assert.strictEqual(scenario.schema_version, 'efa.evaluator-rag.scenario.v1');
  assert.strictEqual(trace.schema_version, 'efa.evaluator-rag.trace.v1');
  assert.strictEqual(report.schema_version, 'efa.evaluator-rag.report.v1');
  assert.strictEqual(verifier.schema_version, 'efa.evaluator-rag.verifier.v1');
...
test('billing readiness scenario rejects launch copy overclaims', () => {
  ...
  assert.ok(playbook.includes('https://github.com/marketplace/efa-tools'));
});
...
test('AgentShield policy exception scenario rejects blanket suppression', () => {
  ...
  for (const blocked of [
    ...
    'editing AgentShield code or policy files from this EFA evaluator run'
  ]) {
  ...
  assert.ok(playbook.includes('npx efa-agentshield scan --format json'));
});
...
test('deep analyzer evidence scenario rejects no-corpus analyzer changes', () => {
  ...
  assert.ok(playbook.includes('../EFA-Tools/src/analyzers/fixtures/deep-analyzer-corpus.ts'));
