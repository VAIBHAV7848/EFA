...
console.log('\n=== Testing EFA 2.0 release surface ===\n');
...
test('business launch copy stays aligned with the rc.1 public surface', () => {
  ...
  assert.ok(source.includes('EFA v2.0.0-rc.1'), 'business launch copy should use the rc.1 release');
  ...
  assert.ok(source.includes('https://github.com/VAIBHAV7848/EFA'), 'business launch copy should include the public repo URL');
  assert.ok(source.includes('https://github.com/VAIBHAV7848/EFA/blob/main/docs/releases/2.0.0-rc.1/release-notes.md'), 'business launch copy should link to the rc.1 release notes');
...
test('announcement drafts avoid live-release claims before publication', () => {
  ...
  for (const relativePath of announcementFiles) {
    ...
    assert.ok(!/EFA v2\.0\.0-rc\.1 is live\./.test(source), `${relativePath} must not claim rc.1 is live before the release gate completes`);
  }
});
...
test('Hermes setup uses release-candidate wording for the rc.1 surface', () => {
  ...
  assert.ok(source.includes('EFA v2.0.0-rc.1 documents the Hermes surface'));
...
test('Hermes setup cross-links adjacent migration and architecture docs', () => {
  ...
  assert.ok(!source.includes('0.5. Generate and review artifacts with `efa migrate plan` /'));
});

test('release docs preserve the EFA/Hermes boundary', () => {
  const releaseNotes = read('docs/releases/2.0.0-rc.1/release-notes.md');
  assert.ok(releaseNotes.includes('EFA is the reusable substrate'));
...
test('preview pack manifest assembles release, Hermes, and publication gates', () => {
  ...
  for (const blocker of [
    ...
    'npm `efa-universal@2.0.0-rc.1`',
    'Claude plugin tag',
    'Codex repo-marketplace distribution evidence',
    'EFA Tools billing/product readiness'
  ]) {
...
test('owner approval packet consolidates the final gated decisions', () => {
  ...
  const hypergrowth = read('docs/releases/2.0.0/efa-2-hypergrowth-release-command-center.md');
  ...
  for (const urlSurface of ['GitHub prerelease URL', 'npm rc package URL', 'Claude plugin tag URL', 'Primary launch video URL', 'EFA Tools billing/readiness URL']) {
...
test('GA roadmap mirrors the current May 19 release evidence', () => {
  const roadmap = read('docs/EFA-2.0-GA-ROADMAP.md');
...
test('rc.1 quickstart gives a clone-to-cross-harness path', () => {
  ...
  assert.ok(quickstart.includes('git clone https://github.com/VAIBHAV7848/EFA.git'));
  assert.ok(quickstart.includes('cd EFA'));
...
test('launch checklist records the efa2 alpha version policy', () => {
  const cargoToml = read('efa2/Cargo.toml');
  const launchChecklist = read('docs/releases/2.0.0-rc.1/launch-checklist.md');
  assert.ok(cargoToml.includes('version = "0.1.0"'));
  assert.ok(launchChecklist.includes('`efa2/Cargo.toml` stays at `0.1.0`'));
  assert.ok(!launchChecklist.includes('confirm whether `efa2/Cargo.toml` moves'));
});
...
test('release video suite manifest gates the content launch lane', () => {
  ...
  for (const marker of [
    ...
    'EFA_VIDEO_SOURCE_ROOT',
    'EFA_VIDEO_RELEASE_SUITE_ROOT',
    'video-use compatible workflow',
    'Self-Eval Gate',
    'Do Not Publish If',
    'renders/efa-2-primary-launch-rough-v1.mp4',
...
test('release video suite public docs do not expose private media paths', () => {
  const releaseVideoDocs = ['docs/releases/2.0.0-rc.1/video-suite-production.md', 'docs/releases/2.0.0/efa-2-hypergrowth-release-command-center.md'];
...
test('publication readiness checklist gates public release actions on evidence', () => {
  ...
  for (const surface of ['GitHub release', 'npm package', 'Claude plugin', 'Codex plugin', 'Codex repo marketplace', 'OpenCode package', 'EFA Tools billing reference', 'Announcement copy']) {
  ...
  assert.ok(may15Evidence.includes('EFA Tools PR #73'));
  assert.ok(may15Evidence.includes('EFA-Tools PR #75'));
  ...
  assert.ok(source.includes('EFA-Tools #92 main CI'));
  assert.ok(source.includes('EFA-Tools #93 main CI'));
...
test('release name and plugin publication checklist freezes rc.1 surfaces', () => {
  ...
  const launchChecklist = read('docs/releases/2.0.0-rc.1/launch-checklist.md');
  const referenceArchitecture = read('docs/EFA-2.0-REFERENCE-ARCHITECTURE.md');

  for (const value of [
    'Ship `v2.0.0-rc.1` as **EFA**',
    '`VAIBHAV7848/EFA`',
    '`efa-universal`',
    '`efa` on npm is occupied',
    '`@VAIBHAV7848/EFA` is unclaimed on npm',
    'Claude plugin',
    'Codex plugin',
...
test('active release identity surfaces use canonical EFA repo URLs', () => {
  const activeFiles = [
    'README.md',
  ...
  const offenders = [];
  const legacyOwner = 'aff' + 'aan-m';
  const legacyRepoLong = 'everything-cl' + 'aude-code';
  const legacyRepoShort = 'E' + 'C' + 'C';
  for (const relativePath of activeFiles) {
    const source = read(relativePath);
    if (source.includes(legacyOwner + '/' + legacyRepoLong) || source.includes(legacyOwner + '/' + legacyRepoShort)) {
      offenders.push(relativePath);
    }
  }
