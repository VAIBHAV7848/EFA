...
const { INLINE_RESOLVE } = require('../../scripts/lib/resolve-efa-root');
...
test('sessions command uses shared inline resolver in all node scripts', () => {
  ...
  assert.strictEqual((sessionsDoc.match(/\['marketplaces','efa'\]/g) || []).length, 6);
  assert.strictEqual((sessionsDoc.match(/\['marketplaces','everything-for-ai'\]/g) || []).length, 6);
  assert.strictEqual((sessionsDoc.match(/\['efa','everything-for-ai'\]/g) || []).length, 6);
});
...
test('skill-health command uses shared inline resolver in all shell snippets', () => {
  ...
  assert.strictEqual((skillHealthDoc.match(/\['marketplaces','efa'\]/g) || []).length, 3);
  assert.strictEqual((skillHealthDoc.match(/\['marketplaces','everything-for-ai'\]/g) || []).length, 3);
  assert.strictEqual((skillHealthDoc.match(/\['efa','everything-for-ai'\]/g) || []).length, 3);
});
...
test('instinct-status command uses shared inline resolver (no stale legacy fallback) (#2037)', () => {
  ...
  assert.strictEqual((instinctStatusDoc.match(/\['marketplaces','efa'\]/g) || []).length, 1);
  assert.strictEqual((instinctStatusDoc.match(/\['marketplaces','everything-for-ai'\]/g) || []).length, 1);
  assert.strictEqual((instinctStatusDoc.match(/\['efa','everything-for-ai'\]/g) || []).length, 1);
...
test('inline resolver covers current and legacy marketplace plugin roots', () => {
  assert.ok(INLINE_RESOLVE.includes("'marketplaces','efa'"));
  assert.ok(INLINE_RESOLVE.includes("'marketplaces','everything-for-ai'"));
