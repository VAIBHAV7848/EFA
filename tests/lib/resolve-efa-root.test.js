/**
 * Tests for scripts/lib/resolve-efa-root.js
 *
 * Covers the EFA root resolution fallback chain:
...
const { resolveEccRoot, INLINE_RESOLVE } = require('../../scripts/lib/resolve-efa-root');
...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'efa-root-test-'));
}
...
function runTests() {
  ...
  if (test('finds current plugin install at ~/.claude/plugins/efa', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['efa']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
  ...
  if (test('finds current plugin install at ~/.claude/plugins/efa@efa', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['efa@efa']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds exact legacy plugin install at ~/.claude/plugins/everything-for-ai', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['everything-for-ai']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds exact legacy plugin install at ~/.claude/plugins/everything-for-ai@everything-for-ai', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['everything-for-ai@everything-for-ai']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds marketplace current plugin install at ~/.claude/plugins/marketplaces/efa', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'efa']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('finds marketplace legacy plugin install at ~/.claude/plugins/marketplaces/everything-for-ai', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'everything-for-ai']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('prefers exact legacy plugin install over plugin cache', () => {
    ...
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'efa']);
      setupPluginCache(homeDir, 'efa', 'VAIBHAV7848', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('discovers plugin root from cache directory', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupPluginCache(homeDir, 'efa', 'VAIBHAV7848', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('handles multiple versions in plugin cache', () => {
    const homeDir = createTempDir();
    try {
      setupPluginCache(homeDir, 'everything-for-ai', 'legacy-org', '1.7.0');
      const expected = setupPluginCache(homeDir, 'efa', 'VAIBHAV7848', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      // Should find one of them (either is valid)
      assert.ok(
        result === expected ||
        result === path.join(homeDir, '.claude', 'plugins', 'cache', 'everything-for-ai', 'legacy-org', '1.7.0'),
