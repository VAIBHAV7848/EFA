/**
 * Tests for allowlisted EFA2 control-pane actions.
 */
...
function runTests() {
  ...
  if (test('builds copyable and executable allowlisted EFA2 actions', () => {
    ...
    assert.strictEqual(sync.cwd, path.join(repoRoot, 'efa2'));
  ...
  if (test('rejects unknown action identifiers', () => {
    assert.throws(
      () => buildControlPaneAction('rm -rf', { repoRoot: '/repo/efa' }),
