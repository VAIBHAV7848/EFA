...
test('dmux adapter normalizes orchestration snapshots into canonical form', () => {
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-recordings-'));
  ...
  try {
    ...
    assert.strictEqual(snapshot.schemaVersion, 'efa.session.v1');
...
test('claude-history adapter loads the latest recorded session', () => {
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-adapter-home-'));
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-recordings-'));
  ...
  fs.writeFileSync(sessionPath, [
    ...
    '**Started:** 09:00',
    '**Last Updated:** 11:30',
    '**Project:** everything-for-ai',
    '**Branch:** feat/session-adapter',
    '**Worktree:** /tmp/efa-worktree',
    '',
    '### Completed',
  ...
  try {
    withHome(homeDir, () => {
      ...
      assert.strictEqual(snapshot.workers[0].worktree, '/tmp/efa-worktree');
...
test('adapter registry routes plan files to dmux and explicit claude targets to history', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-registry-repo-'));
  ...
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-registry-home-'));
  const sessionsDir = path.join(homeDir, '.claude', 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
...
test('adapter registry resolves structured target types into the correct adapter', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-typed-repo-'));
  const planPath = path.join(repoRoot, 'workflow.json');
  fs.writeFileSync(planPath, JSON.stringify({
  ...
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-typed-home-'));
  const sessionsDir = path.join(homeDir, '.claude', 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
    path.join(sessionsDir, '2026-03-13-z9y8x7w6-session.tmp'),
...
test('canonical snapshot validation rejects malformed required fields', () => {
  const invalidCases = [
    ...
    [canonicalSnapshot({ schemaVersion: 'efa.session.v0' }), /Unsupported canonical session schema version/],
...
test('claude history normalization falls back to filename ids and empty metadata defaults', () => {
  ...
  assert.deepStrictEqual(pathOnly.workers[0].intent.seedPaths, ['README.md', 'scripts/efa.js']);
...
test('fallback recordings sanitize paths, use env dirs, and preserve changed history', () => {
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-recordings-env-'));
  const previousRecordingDir = process.env.EFA_SESSION_RECORDING_DIR;

  try {
    process.env.EFA_SESSION_RECORDING_DIR = recordingDir;
    ...
    assert.strictEqual(persisted.schemaVersion, 'efa.session.recording.v1');
    ...
    if (typeof previousRecordingDir === 'string') {
      process.env.EFA_SESSION_RECORDING_DIR = previousRecordingDir;
    } else {
      delete process.env.EFA_SESSION_RECORDING_DIR;
    }
    fs.rmSync(recordingDir, { recursive: true, force: true });
  }
});
...
test('persistence supports skip mode, writer variants, and missing state-store fallback', () => {
  ...
  const noWriterDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-no-writer-'));
  const missingModuleDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-session-missing-module-'));
  try {
