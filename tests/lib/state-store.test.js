/**
 * Tests for the SQLite-backed EFA state store and CLI commands.
 */
...
const EFA_SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'efa.js');
...
const mode = process.env.EFA_FAKE_GH_MODE || 'open';
...
if (args[0] === 'pr' && args[1] === 'list') {
  if (mode === 'empty') write([]);
  else write([
    {
      ...
      url: 'https://github.com/VAIBHAV7848/EFA/pull/3',
    ...
    },
    {
      ...
      url: 'https://github.com/VAIBHAV7848/EFA/pull/4',
  ...
  if (mode === 'empty') write([]);
  else write([
    {
      ...
      url: 'https://github.com/VAIBHAV7848/EFA/issues/9',
...
async function seedStore(dbPath) {
  ...
  store.upsertSession({
    ...
    harness: 'claude',
    state: 'active',
    repoRoot: '/tmp/efa-repo',
    startedAt: '2026-03-15T08:00:00.000Z',
    endedAt: null,
    snapshot: {
      schemaVersion: 'efa.session.v1',
      adapterId: 'dmux-tmux',
      session: {
        id: 'session-active',
        kind: 'orchestrated',
      ...
      workers: [
        {
          ...
          worktree: '/tmp/efa-repo/.worktrees/worker-1',
        },
        {
          id: 'worker-2',
          label: 'Worker 2',
          state: 'idle',
          branch: 'feat/state-store',
          worktree: '/tmp/efa-repo/.worktrees/worker-2',
        },
      ],
      aggregates: {
        workerCount: 2,
  ...
  store.insertSkillRun({
    ...
    skillVersion: '1.0.0',
    sessionId: 'session-recorded',
    taskDescription: 'Outline EFA 2.0 work',
    outcome: 'unknown',
    failureReason: null,
    tokensUsed: 300,
    durationMs: 500,
  ...
  store.insertDecision({
    ...
    rationale: 'Need queryable local state for EFA control plane',
...
async function runTests() {
  ...
  if (await test('creates the default state.db path and applies migrations idempotently', async () => {
    const homeDir = createTempDir('efa-state-home-');

    try {
      const expectedPath = path.join(homeDir, '.claude', 'efa', 'state.db');
  ...
  if (await test('preserves SQLite special database names like :memory:', async () => {
    const tempDir = createTempDir('efa-state-memory-');
  ...
  if (await test('stores sessions and returns detailed session views with workers, skill runs, and decisions', async () => {
    const testDir = createTempDir('efa-state-db-');
  ...
  if (await test('builds an empty status snapshot with null rates and missing install health', async () => {
    const testDir = createTempDir('efa-state-empty-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('tracks linked work items for Linear, GitHub, and handoff progress', async () => {
    const testDir = createTempDir('efa-state-work-items-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const linearItem = store.upsertWorkItem({
        id: 'linear-efa-20-control-plane',
        source: 'linear',
        sourceId: 'EFA-20',
        title: 'Define harness-neutral session/worktree contract',
        status: 'in-progress',
        priority: 'high',
        url: 'https://linear.app/ecctools/issue/EFA-20',
        owner: 'control-plane',
        repoRoot: '/tmp/efa-repo',
        sessionId: 'session-active',
        metadata: {
          project: 'EFA 2.0: Control Plane',
        },
      ...
      store.upsertWorkItem({
        ...
        source: 'handoff',
        sourceId: 'efa-rc1-release-decision-20260511.md',
      ...
      store.upsertWorkItem({
        ...
        status: 'merged',
        priority: 'normal',
        url: 'https://github.com/VAIBHAV7848/EFA/pull/1738',
        owner: 'maintainer',
      ...
      assert.strictEqual(linearItem.id, 'linear-efa-20-control-plane');
      assert.strictEqual(linearItem.metadata.project, 'EFA 2.0: Control Plane');
  ...
  if (await test('normalizes default optional fields and reports warning install health', async () => {
    const testDir = createTempDir('efa-state-defaults-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('rejects invalid limits and unserializable JSON payloads', async () => {
    const testDir = createTempDir('efa-state-errors-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI supports human-readable and --json output', async () => {
    const testDir = createTempDir('efa-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI --exit-code reports attention without suppressing output', async () => {
    const attentionDir = createTempDir('efa-state-attention-');
    const okDir = createTempDir('efa-state-ok-');
  ...
  if (await test('status CLI can emit and write markdown operator snapshots', async () => {
    ...
    try {
      ...
      assert.match(written, /^# EFA Status/m);
  ...
  if (await test('work-items CLI supports upsert, list, show, and close', async () => {
    const testDir = createTempDir('efa-work-items-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      const upsertResult = runNode(WORK_ITEMS_SCRIPT, [
        ...
        'high',
        '--url',
        'https://linear.app/example/issue/EFA-99',
        '--owner',
        'control-plane',
        '--metadata-json',
        '{"project":"EFA 2.0"}',
      ...
      assert.strictEqual(listPayload.items[0].id, 'linear-efa-99');

      const showResult = runNode(WORK_ITEMS_SCRIPT, ['show', 'linear-efa-99', '--db', dbPath]);
      assert.strictEqual(showResult.status, 0, showResult.stderr);
      assert.match(showResult.stdout, /linear\/#EFA-99 in-progress: Ship work item CLI/);

      const closeResult = runNode(WORK_ITEMS_SCRIPT, ['close', 'linear-efa-99', '--db', dbPath, '--json']);
  ...
  if (await test('work-items CLI syncs GitHub PRs and issues into readiness', async () => {
    const testDir = createTempDir('efa-work-items-github-');
    const dbPath = path.join(testDir, 'state.db');
    const binDir = path.join(testDir, 'bin');
    const repo = 'VAIBHAV7848/EFA';

    try {
      const env = {
        EFA_GH_SHIM: createGhShim(binDir),
      };
      ...
      assert.strictEqual(syncPayload.items[0].id, 'github-vaibhav7848-efa-pr-3');
  ...
  if (await test('efa CLI delegates the new status, sessions, and work-items subcommands', async () => {
    const testDir = createTempDir('efa-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const statusResult = runNode(EFA_SCRIPT, ['status', '--db', dbPath, '--json']);
      ...
      const sessionsResult = runNode(EFA_SCRIPT, ['sessions', 'session-active', '--db', dbPath, '--json']);
      ...
      const workItemResult = runNode(EFA_SCRIPT, [
        'work-items',
        'upsert',
        'handoff-roadmap',
        '--db',
      ...
      const delegatedStatusResult = runNode(EFA_SCRIPT, ['status', '--db', dbPath, '--json']);
