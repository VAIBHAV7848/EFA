...
function runTests() {
  ...
  if (test('creates a valid install-state payload', () => {
    const state = createInstallState({
      ...
      installStatePath: '/repo/.cursor/efa-install-state.json',
      request: {
        profile: 'developer',
    ...
    assert.strictEqual(state.schemaVersion, 'efa.install.v1');
  ...
  if (test('writes and reads install-state from disk', () => {
    ...
    const statePath = path.join(testDir, 'efa-install-state.json');
  ...
  if (test('rejects invalid install-state payloads on read', () => {
    ...
    try {
      fs.writeFileSync(statePath, JSON.stringify({ schemaVersion: 'efa.install.v1' }, null, 2));
      assert.throws(
