...
const DESCRIPTORS = [
  ...
  },
  {
    ...
    fn: () => {
      const state = { schemaVersion: 'efa.github.coordination.v1', kind: 'epic', status: 'available' };
      const body = [
      ...
      assert.strictEqual(result.schemaVersion, 'efa.github.coordination.v1');
    },
  },
  {
    group: 'extractCoordinationState',
  ...
  // renderCoordinationState
  {
    ...
    fn: () => {
      const state = {
        schemaVersion: 'efa.github.coordination.v1', kind: 'epic', status: 'available',
      ...
      assert.ok(rendered.includes('<!-- efa-coordination:start -->'), 'Missing start marker');
      assert.ok(rendered.includes('<!-- efa-coordination:end -->'), 'Missing end marker');
  ...
  },
  {
    ...
    fn: () => {
      const state = {
        schemaVersion: 'efa.github.coordination.v1', kind: 'epic', status: 'claimed',
