...
test('buildOrchestrationPlan creates worktrees, branches, and tmux commands', () => {
  const repoRoot = path.join('/tmp', 'efa');
  ...
  assert.ok(
    plan.workerPlans[0].worktreePath.endsWith(path.join('efa-skill-audit-docs-a')),
...
test('normalizeSeedPaths rejects paths outside the repo root', () => {
  assert.throws(
    () => normalizeSeedPaths(['../outside.txt'], '/tmp/efa'),
    /inside repoRoot/
  );
});

test('materializePlan keeps worker instructions inside the worktree boundary', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-orchestrator-test-'));
...
test('overlaySeedPaths copies local overlays into the worker worktree', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-orchestrator-overlay-'));
...
test('executePlan rolls back partial setup when orchestration fails mid-run', () => {
  const plan = {
    ...
    coordinationDir: '/tmp/efa/.orchestration/rollback-test',
    replaceExisting: false,
    workerPlans: [
      {
        workerName: 'Docs',
        workerSlug: 'docs',
        worktreePath: '/tmp/efa-rollback-docs',
        seedPaths: ['commands/multi-workflow.md'],
        gitArgs: ['worktree', 'add', '-b', 'orchestrator-rollback-test-docs', '/tmp/efa-rollback-docs', 'HEAD'],
...
test('executePlan does not mark pre-existing resources for rollback when worktree creation fails', () => {
  const plan = {
    ...
    coordinationDir: '/tmp/efa/.orchestration/rollback-existing',
    replaceExisting: false,
    workerPlans: [
      {
        workerName: 'Docs',
        workerSlug: 'docs',
        worktreePath: '/tmp/efa-existing-docs',
        seedPaths: [],
        gitArgs: ['worktree', 'add', '-b', 'orchestrator-rollback-existing-docs', '/tmp/efa-existing-docs', 'HEAD'],
        launchCommand: 'echo run',
