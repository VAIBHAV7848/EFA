...
function runTests() {
  ...
  if (
    test('returns path containing efa-metrics-', () => {
      const p = getBridgePath('test-session');
      assert.ok(p.includes('efa-metrics-'), `Expected efa-metrics- in path, got: ${p}`);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolveSessionId uses EFA_SESSION_ID env var', () => {
      const original = process.env.EFA_SESSION_ID;
      try {
        process.env.EFA_SESSION_ID = 'env-session-42';
        const result = resolveSessionId();
        assert.strictEqual(result, 'env-session-42');
      } finally {
        if (original === undefined) {
          delete process.env.EFA_SESSION_ID;
        } else {
          process.env.EFA_SESSION_ID = original;
        }
      }
    })
  )
