/**
 * Tests for the local EFA2 control-pane state projection.
 */
...
async function writeSampleEcc2Database(dbPath) {
  ...
  insertSession.run([
    'lead-hermes',
    'Coordinate Hermes desktop and EFA release work',
    'EFA',
    '2.0-control-pane',
    'claude',
    'claude',
    ...
    'running',
    4242,
    '/tmp/efa-worktrees/hermes',
    'efa/hermes-control-pane',
    'main',
    1200,
    800,
    2000,
  ...
  insertEntity.run([
    ...
    '/notes/hermes.md',
    'How VAIBHAV7848 routes Hermes Desktop, Zellij panes, Devin-style delegation, and EFA release control work.',
  ...
  db.run(
    ...
    [
      ...
      3,
      1,
      'Hermes Desktop and EFA should share recall before dispatching work.',
...
async function writeSampleWorkItemsDatabase(dbPath) {
  ...
  insertWorkItem.run([
    ...
    'github-pr',
    '2131',
    'Merge EFA control pane',
    'done',
    'normal',
    'https://github.com/VAIBHAV7848/EFA/pull/2131',
    'VAIBHAV7848',
    '/repo/efa',
    null,
    JSON.stringify({
      branch: 'product/efa2-knowledge-control-pane',
...
async function runTests() {
  ...
  if (await test('builds an operator snapshot from EFA2 SQLite and configured connectors', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-state-'));
    const dbPath = path.join(tempDir, 'efa2.db');
    ...
    try {
      ...
      assert.strictEqual(snapshot.schemaVersion, 'efa.control-pane.snapshot.v1');
  ...
  if (await test('projects state-store work items into agent Kanban summary', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-work-items-'));
    const dbPath = path.join(tempDir, 'efa2.db');
  ...
  if (await test('treats an unreadable optional state-store database as empty work items', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-corrupt-work-items-'));
    const dbPath = path.join(tempDir, 'efa2.db');
  ...
  if (await test('resolves config from explicit db path and TOML connector file', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-config-'));
    const dbPath = path.join(tempDir, 'state.db');
    const configPath = path.join(tempDir, 'efa2.toml');
  ...
  if (await test('prefers the operator home config over stale app-support config', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-precedence-'));
    const homeDir = path.join(tempDir, 'home');
    const homeConfigDir = path.join(homeDir, '.claude');
    const appConfigDir = path.join(homeDir, 'Library', 'Application Support', 'efa2');
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(homeConfigDir, 'efa2.toml'),
  ...
  if (await test('shows configured connectors even when the SQLite database is missing', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-missing-db-'));
  ...
  if (await test('handles an existing SQLite database before EFA2 tables are created', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-empty-db-'));
  ...
  if (await test('handles malformed JSON rows and all session state counters', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efa-control-pane-edge-db-'));
    const dbPath = path.join(tempDir, 'efa2.db');
    ...
    const observations = [
      {
        ...
        priority: 1,
        pinned: false,
        summary: 'Ito should expose market backtesting through EFA tools.',
      },
    ];
