...
function runTests() {
  ...
  if (test('returns false when EFA_DRY_RUN is unset', () => {
    const env = { ...process.env };
    delete env.EFA_DRY_RUN;
  ...
  if (test('returns true when EFA_DRY_RUN=1', () => {
    const env = { ...process.env, EFA_DRY_RUN: '1' };
    const result = spawnSync(process.execPath, [
      '-e',
      'const { isDryRun } = require("./scripts/lib/hook-flags"); process.exit(isDryRun() ? 1 : 0)',
    ], { cwd: path.resolve(__dirname, '..', '..'), env });
  ...
  if (test('returns false when EFA_DRY_RUN=0', () => {
    const env = { ...process.env, EFA_DRY_RUN: '0' };
    const result = spawnSync(process.execPath, [
      '-e',
      'const { isDryRun } = require("./scripts/lib/hook-flags"); process.exit(isDryRun() ? 1 : 0)',
    ], { cwd: path.resolve(__dirname, '..', '..'), env });
  ...
  if (test('skips hook execution and logs preview when EFA_DRY_RUN=1', () => {
  ...
  if (test('executes hook normally when EFA_DRY_RUN is not set', () => {
    const runWithFlags = path.resolve(__dirname, '..', '..', 'scripts', 'hooks', 'run-with-flags.js');
    const hookScript = 'scripts/hooks/doc-file-warning.js';
  ...
  if (test('--dry-run sets EFA_DRY_RUN env var for child commands', () => {
    const eccJs = path.resolve(__dirname, '..', '..', 'scripts', 'efa.js');
