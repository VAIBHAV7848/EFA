...
This guide helps you migrate from Claude Code to OpenCode while using the EFA configuration.
...
### EFA Plugin Hooks Included

The EFA OpenCode configuration includes translated hooks:
...
### 2. Use the EFA OpenCode Configuration
...
│   ├── efa-hooks.ts           # All EFA hooks as plugins
...
---
description: Create implementation plan
agent: everything-for-ai:planner
---
...
### Option 1: Use EFA Configuration Directly
...
This only loads the published EFA OpenCode plugin module (hooks/events and exported plugin tools).
It does **not** automatically inject EFA's full `agent`, `command`, or `instructions` config into your project.

If you want the full EFA OpenCode workflow surface, use the repository's bundled `.opencode/opencode.json` as your base config or copy these pieces into your project:
- `.opencode/commands/`
- `.opencode/prompts/`
...
4. If you installed only `plugin: ["efa-universal"]`, note that npm plugin install does not auto-add EFA commands or agents to your project config
...
- **EFA Configuration**: Report to [github.com/VAIBHAV7848/EFA](https://github.com/VAIBHAV7848/EFA)
