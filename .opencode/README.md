...
> If you installed EFA via npm (e.g. `npm install opencode-efa`), refer to the root README instead.

EFA plugin for OpenCode - agents, commands, hooks, and skills.

## Installation

## Installation Overview

There are two ways to use EFA:

1. **npm package (recommended for most users)**
   Install via npm/bun/yarn and use the `efa-install` CLI to set up rules and agents.
...
This loads the EFA OpenCode plugin module from npm:
- hook/event integrations
- bundled custom tools exported by the plugin

It does **not** auto-register the full EFA command/agent/instruction catalog in your project config. For the full OpenCode setup, either:
...
After installation, the `efa-install` CLI is also available:
...
If you also want to apply the EFA home install
...
- `EFA_HOOK_PROFILE`: `minimal`, `standard` (default), `strict`
- `EFA_DISABLED_HOOKS`: comma-separated hook IDs to disable

## Skills

The default OpenCode config loads 11 curated EFA skills via the `instructions` array:
