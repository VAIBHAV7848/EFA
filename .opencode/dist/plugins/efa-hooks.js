...
/**
 * Read EFA version from package.json
...
export const ECCHooksPlugin = async ({ client, $, directory, worktree, }) => {
    ...
    const log = (level, message) => client.app.log({ body: { service: "efa", level, message } });
    const normalizeProfile = (value) => {
        if (value === "minimal" || value === "strict")
            return value;
        return "standard";
    };
    const currentProfile = normalizeProfile(process.env.EFA_HOOK_PROFILE);
    const disabledHooks = new Set((process.env.EFA_DISABLED_HOOKS || "")
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean));
    const profileOrder = {
    ...
    return {
        ...
        "file.edited": async (event) => {
            ...
            if (hookEnabled("post:edit:format", ["strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
                try {
                    ...
                    log("info", `[EFA] Formatted: ${event.path}`);
                }
                catch (error) {
                    // Prettier not installed or failed - log but continue
                    const errorMessage = error instanceof Error ? error.message : String(error);
                    log("debug", `[EFA] Prettier formatting failed for ${event.path}: ${errorMessage}`);
                }
            }
            ...
            if (hookEnabled("post:edit:console-warn", ["standard", "strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
                try {
                    ...
                    if (result.trim()) {
                        ...
                        log("warn", `[EFA] console.log found in ${event.path} (${lines} occurrence${lines > 1 ? "s" : ""})`);
                    }
                }
                catch {
        ...
        "tool.execute.after": async (input, output) => {
            ...
            if (hookEnabled("post:edit:typecheck", ["strict"]) &&
                ...
                try {
                    ...
                    log("info", "[EFA] TypeScript check passed");
                }
                catch (error) {
                    const err = error;
                    log("warn", "[EFA] TypeScript errors detected:");
                    if (err.stdout) {
            ...
            if (hookEnabled("post:bash:pr-created", ["standard", "strict"]) &&
                ...
                log("info", "[EFA] PR created - check GitHub Actions status");
            }
        },
        /**
        ...
        "tool.execute.before": async (input) => {
            ...
            if (hookEnabled("pre:bash:git-push-reminder", "strict") &&
                ...
                log("info", "[EFA] Remember to review changes before pushing: git diff origin/main...HEAD");
            }
            ...
            if (hookEnabled("pre:write:doc-file-warning", ["standard", "strict"]) &&
                ...
                if (filePath.match(/\.(md|txt)$/i) &&
                    ...
                    log("warn", `[EFA] Creating ${filePath} - consider if this documentation is necessary`);
                }
            }
            ...
            if (hookEnabled("pre:bash:tmux-reminder", "strict") && input.tool === "bash") {
                ...
                if (cmd.match(/^(npm|pnpm|yarn|bun)\s+(install|build|test|run)/) ||
                    ...
                    log("info", "[EFA] Long-running command detected - consider using background execution");
                }
            }
        },
        /**
        ...
        "session.created": async () => {
            if (!hookEnabled("session:start", ["minimal", "standard", "strict"]))
                return;
            log("info", `[EFA] Session started - profile=${currentProfile}`);
            // Check for project-specific context files
            if (hasProjectFile("CLAUDE.md")) {
                log("info", "[EFA] Found CLAUDE.md - loading project context");
            }
        },
        /**
         * Session Idle Hook
        ...
        "session.idle": async () => {
            ...
            if (editedFiles.size === 0)
                return;
            log("info", "[EFA] Session idle - running console.log audit");
            ...
            if (totalConsoleLogCount > 0) {
                log("warn", `[EFA] Audit: ${totalConsoleLogCount} console.log statement(s) in ${filesWithConsoleLogs.length} file(s)`);
                filesWithConsoleLogs.forEach((f) => log("warn", `  - ${f}`));
                log("warn", "[EFA] Remove console.log statements before committing");
            }
            else {
                log("info", "[EFA] Audit passed: No console.log statements found");
            }
            // Desktop notification (cross-platform)
            try {
                if (process.platform === "darwin") {
                    // macOS
                    await $ `osascript -e 'display notification "Task completed!" with title "OpenCode EFA"' 2>/dev/null`;
                }
                else if (process.platform === "win32") {
                    // Windows - PowerShell notification
                    await $ `powershell -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Task completed!', 'OpenCode EFA', 'OK', 'Information')" 2>/dev/null`;
                }
                else if (process.platform === "linux") {
                    // Linux - notify-send (requires libnotify)
                    await $ `notify-send "OpenCode EFA" "Task completed!" 2>/dev/null`;
                }
            }
            catch (error) {
                // Notification not supported or failed - log but continue
                const errorMessage = error instanceof Error ? error.message : String(error);
                log("debug", `[EFA] Desktop notification failed: ${errorMessage}`);
            }
        ...
        "session.deleted": async () => {
            if (!hookEnabled("session:end-marker", ["minimal", "standard", "strict"]))
                return;
            log("info", "[EFA] Session ended - cleaning up");
            editedFiles.clear();
            clearChanges();
        ...
        "todo.updated": async (event) => {
            ...
            if (total > 0) {
                log("info", `[EFA] Progress: ${completed}/${total} tasks completed`);
            }
        },
        /**
         ...
         * Action: Sets PROJECT_ROOT, PACKAGE_MANAGER, DETECTED_LANGUAGES, EFA_VERSION
         */
        "shell.env": async () => {
            const env = {
                EFA_VERSION: getECCVersion(),
                EFA_PLUGIN: "true",
                EFA_HOOK_PROFILE: currentProfile,
                EFA_DISABLED_HOOKS: process.env.EFA_DISABLED_HOOKS || "",
         ...
         * Action: Push EFA context block and custom compaction prompt
         */
        "experimental.session.compacting": async () => {
            const contextBlock = [
                "# EFA Context (preserve across compaction)",
                "",
                "## Active Plugin: EFA v2.0.0",
        ...
        "permission.ask": async (event) => {
            log("info", `[EFA] Permission requested for: ${event.tool}`);
            try {
                ...
                if (["read", "glob", "grep", "search", "list"].includes(event.tool)) {
                    log("debug", `[EFA] Auto-approved read-only tool: ${event.tool}`);
                ...
                if (event.tool === "bash" && /^(npx )?(@biomejs\/biome|prettier|black|gofmt|rustfmt|swift-format)/.test(cmd)) {
                    log("debug", `[EFA] Auto-approved formatter: ${cmd}`);
                ...
                if (event.tool === "bash" && /^(npm test|npx vitest|npx jest|pytest|go test|cargo test)/.test(cmd)) {
                    log("debug", `[EFA] Auto-approved test execution: ${cmd}`);
                    return { approved: true, reason: "Test execution" };
                }
                // Everything else: let user decide
                log("debug", `[EFA] Permission requires user approval: ${event.tool}`);
                return { approved: undefined };
            }
            catch (error) {
                // Error in permission handling - log and deny for safety
                const errorMessage = error instanceof Error ? error.message : String(error);
                log("error", `[EFA] Permission handling error for ${event.tool}: ${errorMessage}`);
...
//# sourceMappingURL=efa-hooks.js.map