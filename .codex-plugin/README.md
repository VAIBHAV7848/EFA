# .codex-plugin — Codex Native Plugin for EFA

This directory contains the **Codex plugin manifest** for EFA.
...
codex plugin marketplace add /absolute/path/to/EFA
```

The marketplace entry points at `plugins/efa/` — Codex does not discover
...
[#2128](https://github.com/VAIBHAV7848/EFA/issues/2128)). That thin plugin folder
...
`efa` from the plugin directory.

After install, `codex plugin list` is only a registration check. From an EFA
...
repository shape: `plugins/efa/.codex-plugin/plugin.json`,
`plugins/efa/skills/`, and the supporting README/assets. Until that listing is
...
The installed plugin registers under the short slug `efa` so tool and command names
...
- EFA is moving to a skills-first workflow surface. Legacy `commands/` remain for
