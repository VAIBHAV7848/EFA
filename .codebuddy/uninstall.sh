...
do_uninstall() {
    ...
    echo "EFA CodeBuddy Uninstaller"
    echo "=========================="
    echo ""
    ...
    if [ ! -f "$MANIFEST" ]; then
        echo "Warning: No manifest file found (.efa-manifest)"
        echo ""
        echo "This could mean:"
        echo "  1. EFA was installed with an older version without manifest support"
    ...
    echo "Found manifest file - will only remove files installed by EFA"
    echo ""
    read -p "Are you sure you want to uninstall EFA from $CODEBUDDY_DIR? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Uninstall cancelled."
        exit 0
