#!/usr/bin/env node
/**
 * EFA CodeBuddy Uninstaller (Cross-platform Node.js version)
 * Uninstalls Everything For Ai workflows from a CodeBuddy project.
 *
 * Usage:
...
async function doUninstall() {
  ...
  console.log('EFA CodeBuddy Uninstaller');
  console.log('==========================');
  console.log('');
  ...
  const manifest = path.join(codebuddyFullPath, '.efa-manifest');

  // Handle missing manifest
  if (!fs.existsSync(manifest)) {
    console.log('Warning: No manifest file found (.efa-manifest)');
    console.log('');
    console.log('This could mean:');
    console.log('  1. EFA was installed with an older version without manifest support');
  ...
  console.log('Found manifest file - will only remove files installed by EFA');
  console.log('');

  const confirmed = await promptConfirm(`Are you sure you want to uninstall EFA from ${codebuddyDirName}? (y/N) `);
  if (!confirmed) {
    console.log('Uninstall cancelled.');
    process.exit(0);
  }
