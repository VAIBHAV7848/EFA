#!/bin/bash
#
# EFA CodeBuddy Installer
# Installs Everything For Ai workflows into a CodeBuddy project.
#
# Usage:
...
# Locate the EFA repo root by walking up from SCRIPT_DIR to find the marker
...
if [ -z "$REPO_ROOT" ]; then
    echo "Error: Cannot locate the EFA repository root."
    echo "This script must be run from within the EFA repository's .codebuddy/ directory."
    exit 1
fi
...
do_install() {
    ...
    MANIFEST="$codebuddy_full_path/.efa-manifest"
    touch "$MANIFEST"
    ...
    ensure_manifest_entry "$MANIFEST" ".efa-manifest"
    ...
    echo "  3. Enjoy the EFA workflows!"
    echo ""
