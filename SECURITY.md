...
- <https://github.com/VAIBHAV7848/EFA/security/advisories/new>

You can also email **<security@efa.tools>**.
...
- the `VAIBHAV7848/EFA` repository
- the `efa-universal` npm package
- EFA plugin, install, repair, dashboard, hook, rule, skill, MCP, and command surfaces shipped from this repository
- GitHub Actions workflows and release automation in this repository
- the EFA Tools GitHub App integration points documented by this repository
- AgentShield usage docs when they are embedded here. AgentShield code issues belong in <https://github.com/VAIBHAV7848/agentshield>

## Official Distribution Surfaces

Official EFA surfaces are:

- GitHub repo: <https://github.com/VAIBHAV7848/EFA>
- npm package: `efa-universal`
- GitHub App: <https://github.com/apps/efa-tools>
- marketplace/plugin slug: `efa@efa`
- website: <https://efa.tools>

Official AgentShield surface:

- npm package: `efa-agentshield`
- GitHub repo: <https://github.com/VAIBHAV7848/agentshield>

The following packages have been observed using EFA repository metadata but are **not maintained by EFA**:

- `@chil_ntl/efa-cli`
- `efa-100xprompt-plugin`

Treat any package not listed under official surfaces as unofficial until verified. Do not install packages named `opencode-efa`, `everything-for-ai`, or other EFA-like aliases unless this repository explicitly documents them as official.

GitHub dependency graph may also show Go module aliases such as `github.com/VAIBHAV7848/EFA` or historical repository paths. EFA is not currently distributed as a supported Go module.
...
- screenshots, stale line numbers, or reports against `VAIBHAV7848/EFA` that do not reproduce on current `VAIBHAV7848/EFA`
- self-XSS or social engineering with no repository-controlled exploit path
- dependency graph/package metadata confusion without an install path to an official EFA package
- vulnerabilities in third-party packages unless EFA pins, installs, or executes them in a way that creates extra impact
...
EFA treats supply-chain exposure as a first-class security surface.
...
- Package metadata should point at `VAIBHAV7848/EFA`, not historical repo paths.
...
EFA runs inside agent harnesses that may inject ephemeral client-side system reminders into the model input on every turn. These blocks are not automatically repository-carried payloads.
...
- **AgentShield:** `npx efa-agentshield scan`
