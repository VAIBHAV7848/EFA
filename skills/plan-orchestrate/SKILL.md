...
description: Read a plan document, decompose it into steps, design a per-step agent chain from the EFA catalogue, and emit ready-to-paste /orchestrate custom prompts. Generative only — never invokes /orchestrate itself. Use when the user has a multi-step plan and wants to drive it through orchestrate without composing chains by hand.
metadata:
  origin: EFA
---
...
## EFA install form and namespacing
...
Let `<claude-home>` denote the Everything For Ai home directory: `~/.claude` on macOS/Linux, `%USERPROFILE%\.claude` on Windows. Resolve it the way the host platform resolves the user home directory (do not hardcode `~`).

| Form | Detection | `{ORCH_CMD}` | Agent name format |
|---|---|---|---|
| Plugin install (1.9.0+) | `<claude-home>/plugins/marketplaces/everything-for-ai/` exists | `/everything-for-ai:orchestrate` | `everything-for-ai:<name>` |
| Legacy bare install | Above absent; agent files under `<claude-home>/agents/` | `/orchestrate` | `<name>` |

Why this matters: under the plugin install, agents register as `everything-for-ai:tdd-guide`. Bare names force fuzzy matching, which fails intermittently under parallel calls. Under legacy, the prefixed forms are not registered and fail outright.
...
### Phase 0 — Detect EFA mode + language

1. Read `<plan-doc-path>`. If missing or empty, report and stop.
2. Detect EFA install form once and freeze it into `EFA_MODE`. Algorithm (run in order, stop at the first match):
   1. If `<claude-home>/plugins/marketplaces/everything-for-ai/` exists → `EFA_MODE=plugin`.
   2. Else if `<claude-home>/agents/` exists and contains at least one EFA agent file (e.g. `tdd-guide.md`, `code-reviewer.md`) → `EFA_MODE=legacy`.
   3. Else → default to `EFA_MODE=legacy` and emit a one-line warning at the top of the output: `> Warning: could not detect EFA install; defaulting to legacy form. If you use the plugin install, edit the prefixes manually.`
...
5. **Normalize any agent names declared in the plan**: if the plan text references agents by their plugin-prefixed form (e.g. `everything-for-ai:tdd-guide`), strip the prefix to get the bare catalogue name before validating or composing chains. Re-prefixing happens only at output time per `EFA_MODE` (Phase 4). Never let a pre-prefixed name flow into chain composition — it would double-prefix in plugin mode.
...
Emit Markdown using **the form determined by `EFA_MODE`**. The output uses one form throughout — every `{ORCH_CMD}` and every agent name is rendered with the matching prefix from Phase 0. **Do not emit both forms; do not include "this is plugin form" / "strip the prefix" instructions in the rendered output.**

Concrete rendering rules:

- `{ORCH_CMD}` = `/everything-for-ai:orchestrate` under `plugin`, `/orchestrate` under `legacy`.
- `{AGENT(name)}` = `everything-for-ai:<name>` under `plugin`, `<name>` under `legacy`.
...
- [ ] Every agent in every chain comes from the catalogue (after stripping any `everything-for-ai:` prefix that appeared in the plan; see Phase 0 step 5).
...
- **Plan declares agents** (rare): first **strip any `everything-for-ai:` prefix** to get the bare catalogue name (Phase 0 step 5), then validate against the catalogue. Replace invalid agents and explain under "Chain rationale". The bare name is re-prefixed at output time per `EFA_MODE`.
...
```bash
/everything-for-ai:orchestrate custom "everything-for-ai:tdd-guide,everything-for-ai:database-reviewer,everything-for-ai:python-reviewer,everything-for-ai:security-reviewer" "[Plan: docs/plan/example-feature.md#step-2] Implement EncryptedString SQLAlchemy type and migrate UserProfile.birth_datetime/location columns; key from ENV APP_DB_KEY; Acceptance: encrypt/decrypt roundtrip tests pass; alembic upgrade/downgrade clean on empty DB; no plaintext in DB after migrate; Out of scope: cross-tenant profile sharing logic"
```
````

### Example 2 — Legacy mode, same step

If `EFA_MODE=legacy` were detected, the same step would be emitted as a single uniform command (no plugin-prefixed forms anywhere in the output):
