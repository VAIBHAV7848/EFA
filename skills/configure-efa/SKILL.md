---
name: configure-efa
description: Interactive installer for Everything For Ai — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
metadata:
  origin: EFA
---
...
An interactive, step-by-step installation wizard for the Everything For Ai project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.
...
`claude-api` is an Anthropic canonical skill. Install it from [`anthropics/skills`](https://github.com/anthropics/skills) when you want the official Claude API workflow instead of an EFA-bundled copy.
...
cp -r $EFA_ROOT/rules/typescript $TARGET/rules/typescript   # if selected
cp -r $EFA_ROOT/rules/python $TARGET/rules/python            # if selected
cp -r $EFA_ROOT/rules/golang $TARGET/rules/golang            # if selected
```
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source EFA repository (`$EFA_ROOT/`).
