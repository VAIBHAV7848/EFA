---
name: agent-sort
description: Build an evidence-backed EFA install plan for a specific repo by sorting skills, commands, rules, hooks, and extras into DAILY vs LIBRARY buckets using parallel repo-aware review passes. Use when EFA should be trimmed to what a project actually needs instead of loading the full bundle.
metadata:
  origin: EFA
---
...
Use this skill when a repo needs a project-specific EFA surface instead of the default full install.

The goal is not to guess what "feels useful." The goal is to classify EFA components with evidence from the actual codebase.

## When to Use

- A project only needs a subset of EFA and full installs are too noisy
...
- Prefer EFA-native surfaces; do not introduce a second install system
