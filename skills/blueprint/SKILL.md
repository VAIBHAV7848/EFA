...
This skill ships with Everything For Ai. No separate installation is needed when EFA is installed.

### Full EFA install

If you are working from the EFA repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the EFA diff before updating:
...
If you are vendoring only this skill outside the full EFA install, copy the reviewed file from the EFA repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed EFA commit rather than running `git pull`.
