...
This is the EFA-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Prefer EFA-native workflows over generic "install another plugin" advice when EFA can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in EFA
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent EFA workspaces
...
- existing EFA skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but EFA lacks a billing-operator skill"
- "Google Drive is connected, but there is no EFA-native Google Workspace operator workflow"
...
2. whether EFA already has parity
3. whether EFA only has primitives
4. whether EFA is missing the workflow entirely

### Phase 3: Turn Gaps Into EFA Decisions

For every real gap, recommend the correct EFA-native shape:

| Gap Type | Preferred EFA Shape |
|----------|---------------------|
...
2. **Parity**
   - where EFA already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but EFA lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete EFA-native additions, ordered by impact
...
- If EFA already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what EFA should own next.
