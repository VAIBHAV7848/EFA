...
CONFIG_DIR="$(_efa_resolve_homunculus_dir)"
...
#   - EFA observing its own Haiku observer sessions (self-loop)
#   - EFA observing other tools' automated sessions
...
# (EFA_HOOK_PROFILE=minimal, EFA_SKIP_OBSERVE=1, agent_id, path exclusions).
...
[ "${EFA_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${EFA_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_EFA_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_EFA_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_EFA_SKIP_PATHS="${EFA_OBSERVE_SKIP_PATHS:-observer-sessions,.claude-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _EFA_SKIP_ARRAY <<< "$_EFA_SKIP_PATHS"
  for _pattern in "${_EFA_SKIP_ARRAY[@]}"; do
...
SIGNAL_EVERY_N="${EFA_OBSERVER_SIGNAL_EVERY_N:-20}"
