...
ANALYSIS_COOLDOWN="${EFA_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${EFA_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${EFA_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set EFA_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${EFA_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  observer_tmp_dir="${PROJECT_DIR}/.observer-tmp"
  mkdir -p "$observer_tmp_dir"
  analysis_file="$(mktemp "${observer_tmp_dir}/efa-observer-analysis.XXXXXX.jsonl")"
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/efa-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${EFA_OBSERVER_TIMEOUT_SECONDS:-120}"
  ...
  if [ -n "${EFA_OBSERVER_MAX_TURNS:-}" ]; then
    max_turns="${EFA_OBSERVER_MAX_TURNS}"
  else
  ...
  # but an explicit EFA_OBSERVER_MAX_TURNS override may be non-numeric, empty, or too
  ...
  EFA_SKIP_OBSERVE=1 EFA_HOOK_PROFILE=minimal claude --model haiku --max-turns "$max_turns" --print \
