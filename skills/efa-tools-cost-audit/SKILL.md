---
name: efa-tools-cost-audit
description: Evidence-first EFA Tools burn and billing audit workflow. Use when investigating runaway PR creation, quota bypass, premium-model leakage, duplicate jobs, or GitHub App cost spikes in the EFA Tools repo.
metadata:
  origin: EFA
---
...
Use this skill when the user suspects the EFA Tools GitHub App is burning cost, over-creating PRs, bypassing usage limits, or routing free users into premium analysis paths.

This is a focused operator workflow for the sibling [EFA-Tools](../../EFA-Tools) repo. It is not a generic billing skill and it is not a repo-wide code review pass.

## Skill Stack

Pull these EFA-native skills into the workflow when relevant:
...
- user says EFA Tools burn rate, PR recursion, over-created PRs, usage-limit bypass, or premium-model leakage
- the task is in the sibling `EFA-Tools` repo and depends on webhook handlers, queue workers, usage reservation, PR creation logic, or paid-gate enforcement
...
- work in the sibling `EFA-Tools` repo, not in `everything-for-ai`
...
- switch into the sibling `EFA-Tools` repo
