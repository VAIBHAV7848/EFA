---
name: hermes-imports
description: Convert local Hermes operator workflows into sanitized EFA skills and release-pack artifacts. Use when preparing a Hermes workflow for public EFA reuse without leaking private workspace state, credentials, or local-only paths.
metadata:
  origin: EFA
---
...
Use this skill when turning a repeated Hermes workflow into something safe to ship in EFA.

Hermes is the operator shell. EFA is the reusable workflow layer. Imports should move stable patterns from Hermes into EFA without moving private state.

## When To Use

- A Hermes workflow has repeated enough times to become reusable.
- A local operator prompt should become a public EFA skill.
